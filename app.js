...
async function canAddGround() {
    ...
    // Both venue owners and plot owners can add grounds freely
...
async function handleAddGround(e) {
    ...
    if (groundImages.length < 3) {
        showToast('Please upload at least 3 ground photos', 'error');
        return;
    }
    ...
    try {
        ...
        document.getElementById('owner-qr-result').innerHTML = `
            <div style="color: var(--success); text-align: center;">
                ...
                <p>${booking.venueName} - ${booking.groundName}</p>
                <p>${booking.date} | ${booking.slotTime}</p>
                <p>Entry allowed</p>
            </div>
        `;
                    ...
                    <div class="modal-body">
                        <div class="payment-details-card">
                            ...
                            <div class="payment-note-box">
                                ...
                                <span>Payment is secure and will be held until match completion</span>
                            </div>
                        </div>
                    </div>
                </div>
...
async function processMatchPayment(matchId, match, amount, upiApp) {
    ...
    try {
        const initiatePayment = functions.httpsCallable('initiateMatchPayment');
        const result = await initiatePayment({
            matchId: matchId,
            ...
            creatorUpiId: match.creatorUpiId || ''
        });
        
        if (result.data.success) {
            sessionStorage.setItem('pendingMatchPayment', JSON.stringify({
                matchId: matchId,
...
function selectSlot(slot) {
    ...
    if (!currentGround) {
        showToast('Ground information not found', 'error');
        return;
    }
...
async function initiatePhonePePayment(bookingDetails) {
    ...
    try {
        const initiatePayment = functions.httpsCallable('initiateBookingPayment');
    ...
    try {
        ...
        const statsHtml = `
            <div class="tournament-stats-bar">
                ...
                <div class="tournament-stat-chip">
                    <i class="fas fa-trophy"></i>
                    <span>Total Prize: <strong>${formatCurrency(tournaments.reduce((sum, t) => sum + (t.prizeAmount || 0), 0))}</strong></span>
                </div>
            </div>
        `;
                    ...
                    <div class="tournament-info-grid">
                        ...
                        <div class="tournament-info-item">
                            ...
                            <div>
                                ...
                                <span class="tournament-info-value">${escapeHtml(currentTournament.venueAddress)}</span>
                            </div>
                        </div>
                        <div class="tournament-info-item">
                            <i class="fas fa-calendar-alt"></i>
...
function showTournamentRegistration(tournamentId) {
    ...
    const tournament = currentTournament || { teamSize: 11 };
        ...
        <div class="registration-form">
            ...
            <form id="tournament-registration-form">
                <div class="form-group">
                    ...
                    <input type="text" id="team-name" class="modal-input" required placeholder="Enter your team name">
                </div>
                
                <div class="form-group">
                    <label>Team Captain</label>
                    <input type="text" id="captain-name" class="modal-input" value="${currentUser.name}" required>
                </div>
                
                <div class="form-group">
                    <label>Captain Phone</label>
                    <input type="tel" id="captain-phone" class="modal-input" value="${currentUser.phone || ''}" required>
                </div>
                
                <div class="form-group">
                    <label>Team Members (${tournament.teamSize} players including captain)</label>
                    <div id="player-list" class="player-list">
                        ${Array(tournament.teamSize - 1).fill().map((_, i) => `
                            <div class="player-item">
                                <input type="text" class="modal-input" placeholder="Player ${i + 2} Name" required>
                            </div>
                        `).join('')}
                    </div>
                </div>
                ...
                <div class="form-group">
                    ...
                    <input type="tel" id="contact-number" class="modal-input" value="${currentUser.phone || ''}" required>
                </div>
    ...
    document.getElementById('tournament-registration-form').addEventListener('submit', async (e) => {
        e.preventDefault();
        await processTournamentRegistration(tournamentId);
    });
...
function showTournamentRegistration(tournamentId) {
    ...
    db.collection(COLLECTIONS.TOURNAMENTS).doc(tournamentId).get()
        .then(async (doc) => {
                ...
                <div class="tournament-registration-container">
                    <div class="registration-card">
                        <div class="registration-header">
                            ...
                            <p>Join the competition and showcase your skills</p>
                        </div>
                        
                        <div class="tournament-info-banner">
                            <div class="info-banner-item">
                                <i class="fas fa-tag"></i>
                                <span class="info-banner-label">Tournament</span>
                                <span class="info-banner-value">${escapeHtml(tournament.tournamentName)}</span>
                            </div>
                            <div class="info-banner-item">
                                <i class="fas fa-calendar-alt"></i>
                                <span class="info-banner-label">Date</span>
                                <span class="info-banner-value">${formattedDate}</span>
                            </div>
                            <div class="info-banner-item">
                                <i class="fas fa-users"></i>
                                <span class="info-banner-label">Spots Left</span>
                                <span class="info-banner-value ${spotsLeft <= 5 ? 'urgent-text' : ''}">${spotsLeft}/${tournament.maxTeams}</span>
                            </div>
                            <div class="info-banner-item">
                                <i class="fas fa-rupee-sign"></i>
                                <span class="info-banner-label">Entry Fee</span>
                                <span class="info-banner-value">${formatCurrency(tournament.entryFee)}</span>
                            </div>
                        </div>
                        
                        <form id="tournament-registration-form" class="registration-form-modern">
                            <div class="form-section-modern">
                                <div class="form-section-title-modern">
                                    <i class="fas fa-users"></i>
                                ...
                                <div class="form-group-modern-reg">
                                    ...
                                    <div class="form-hint-modern">
                                        <i class="fas fa-info-circle"></i> Choose a unique and catchy team name
                                    </div>
                                </div>
                                
                                <div class="form-group-modern-reg">
                                    <label><i class="fas fa-crown"></i> Team Captain *</label>
                                    <input type="text" id="captain-name" class="form-input-modern-reg" 
                                           value="${escapeHtml(currentUser.name)}" required>
                                </div>
                                
                                <div class="form-group-modern-reg">
                                    <label><i class="fas fa-phone"></i> Captain's Phone *</label>
                            ...
                            <div class="form-section-modern">
                                <div class="form-section-title-modern">
                                    <i class="fas fa-user-friends"></i>
                                    <span>Team Members (${tournament.teamSize} players total)</span>
                                </div>
                                <div class="player-list-modern" id="player-list-container">
                                    ${Array(tournament.teamSize - 1).fill().map((_, i) => `
                                        <div class="player-item-modern" data-player-index="${i}">
                                            <div class="player-input-wrapper">
                                                <span class="player-number">${i + 2}</span>
                                ...
                                ${tournament.teamSize > 11 ? `
                                    <button type="button" class="add-player-btn" id="add-more-players">
                                        <i class="fas fa-plus-circle"></i> Add More Players
                                    </button>
                                ` : ''}
                                <div class="form-hint-modern" style="margin-top: var(--space-md);">
                                    <i class="fas fa-users"></i> Including captain, you need ${tournament.teamSize} players
                                </div>
                            </div>
                            ...
                            <div class="form-section-modern">
                                ...
                                <div class="form-group-modern-reg">
                                    ...
                                    <input type="tel" id="contact-number" class="form-input-modern-reg" 
                                           value="${escapeHtml(currentUser.phone || '')}" 
                                           placeholder="Number for tournament updates" required>
                                    <div class="form-hint-modern">
                                        <i class="fas fa-bell"></i> We'll send important updates to this number
                                    </div>
                                </div>
                            </div>
                            
                            <div class="registration-summary-modern">
                                ...
                                <div class="summary-row">
                                    ...
                                    <span>${formatCurrency(tournament.entryFee * 0.20)}</span>
                                </div>
                            ...
                            <label class="terms-checkbox-modern">
                                ...
                                <label for="agree-terms-reg">
                                    ...
                                    <a href="#" id="tournament-terms-link">Tournament Rules & Regulations</a>
                                </label>
                            </label>
            ...
            // Add event listeners
            document.getElementById('tournament-registration-form').addEventListener('submit', (e) => {
                e.preventDefault();
                processTournamentRegistration(tournamentId);
            });
            
            document.getElementById('add-more-players')?.addEventListener('click', addMorePlayers);
...
async function processTournamentRegistration(tournamentId) {
    ...
    if (!captainName) {
        ...
        document.getElementById('captain-name')?.focus();
        return;
    }
    
    if (!captainPhone) {
        showToast('Please enter captain phone number', 'error');
        document.getElementById('captain-phone')?.focus();
        return;
    }
    ...
    try {
        ...
        // Show success modal before payment
        showRegistrationSuccessModal(tournamentData, registrationId, teamName);
...
function showTournamentPayment(tournament, registrationId, teamName) {
            ...
            <div class="payment-card">
                ...
                <div class="fee-breakdown-card">
                    ...
                    <div class="payment-note-modern" style="margin-top: var(--space-lg);">
                        <i class="fas fa-info-circle"></i>
                        <span>The platform fee helps us maintain and improve our services</span>
                    </div>
                </div>
                ...
                <div class="upi-payment-section">
                    ...
                    <p class="upi-payment-subtitle">Choose your preferred UPI app to complete payment</p>
                    
                    <div class="upi-apps-grid-modern">
                        <div class="upi-app-modern" data-upi="phonepe@ybl">
                            <img src="https://upload.wikimedia.org/wikipedia/commons/thumb/7/78/PhonePe_Logo.svg/1200px-PhonePe_Logo.svg.png" 
                                 alt="PhonePe" class="upi-logo-modern">
                            <span>PhonePe</span>
                        </div>
                        <div class="upi-app-modern" data-upi="okhdfcbank">
                            <img src="https://upload.wikimedia.org/wikipedia/commons/thumb/2/24/Google_Pay_Logo.svg/1200px-Google_Pay_Logo.svg.png" 
                                 alt="Google Pay" class="upi-logo-modern">
                            <span>Google Pay</span>
                        </div>
                        <div class="upi-app-modern" data-upi="paytm@paytm">
                            <img src="https://upload.wikimedia.org/wikipedia/commons/thumb/2/24/Paytm_Logo_%28standalone%29.svg/1200px-Paytm_Logo_%28standalone%29.svg.png" 
                                 alt="Paytm" class="upi-logo-modern">
                            <span>Paytm</span>
                        </div>
                        <div class="upi-app-modern" data-upi="okaxis">
                            <img src="https://upload.wikimedia.org/wikipedia/commons/thumb/e/e1/Amazon_Pay_Logo.svg/1200px-Amazon_Pay_Logo.svg.png" 
                                 alt="Amazon Pay" class="upi-logo-modern">
                            <span>Amazon Pay</span>
                        </div>
                    </div>
                    ...
                    <div class="payment-note-modern">
                        <i class="fas fa-shield-alt"></i>
                        <span>Your payment is secured with 128-bit encryption</span>
                    </div>
                </div>
    ...
    // Add click handlers for UPI apps
    document.querySelectorAll('.upi-app-modern').forEach(app => {
        app.addEventListener('click', function() {
            const upiId = this.dataset.upi;
            initiateTournamentPayment(tournament, registrationId, teamName, upiId);
        });
    });
...
async function initiateTournamentPayment(tournament, registrationId, teamName, upiApp) {
    ...
    try {
        const initiatePayment = functions.httpsCallable('initiateTournamentPayment');
        const result = await initiatePayment({
...
function showPaymentSuccessModal(registrationId, amount) {
    ...
    if (!modal) {
        const modalHtml = `
            <div id="payment-success-modal" class="modal">
                <div class="modal-content" style="max-width: 400px;">
                    <div class="modal-header">
                        <h3><i class="fas fa-check-circle" style="color: var(--success);"></i> Payment Successful!</h3>
                        <button class="close-btn" id="close-payment-success-modal">&times;</button>
                    </div>
                    <div class="modal-body">
                        <div class="registration-success-modal">
                            <div class="success-icon-large" style="background: linear-gradient(135deg, var(--success), #059669);">
                                <i class="fas fa-check-circle"></i>
                            </div>
                            <h3 class="success-title">Registration Confirmed!</h3>
                            <p class="success-message">Your payment of ${formatCurrency(amount)} was successful. You are now officially registered for the tournament.</p>
                            ...
                            <div class="success-actions">
                                <button class="btn-primary" id="view-registration-confirmation">
                                    <i class="fas fa-eye"></i> View Registration
                                </button>
                                <button class="btn-secondary" id="go-to-home">
    ...
    try {
        ...
        if (snapshot.empty) {
            ...
            for (const doc of snapshot.docs) {
                ...
                html += `
                    <div class="tournament-card-modern" data-tournament-id="${doc.id}">
                        <div class="tournament-card-content">
                            <div class="tournament-header-section">
                                <div class="tournament-info">
                                    <div class="tournament-icon">
                            ...
                            <div class="tournament-actions">
                                <button class="tournament-btn tournament-btn-secondary" onclick="viewTournamentDetails('${doc.id}')">
                                    <i class="fas fa-eye"></i> View Details
                                </button>
                                ${pendingRegistrations > 0 ? `
                                    <button class="tournament-btn tournament-btn-primary" onclick="showTournamentRegistrations('${doc.id}')">
...
function updateEarningsPreview(price) {
    ...
    document.querySelector('.customer-price').textContent = `₹${price.toLocaleString()}`;
    document.querySelector('.platform-fee').textContent = `₹${platformFee.toLocaleString()}`;
    document.querySelector('.owner-earning').textContent = `₹${ownerEarning.toLocaleString()}`;
}
...
async function handleAddGround(e) {
    ...
    const groundAddress = document.getElementById('ground-address-input').value.trim();
    const fileInput = document.getElementById('ground-images');
    const groundImages = fileInput.files;
    ...
    try {
        ...
        for (let i = 0; i < groundImages.length; i++) {
            ...
            uploadStatus.textContent = `Uploading ${uploaded} of ${groundImages.length} photos...`;
        }
...
async function loadOwnerEarnings(container) {
    ...
    try {
        ...
        bookingsSnapshot.forEach(doc => {
            ...
            if (booking.payoutStatus === BOOKING_STATUS.PAYOUT_PENDING || !booking.payoutStatus) {
                pendingPayout += ownerAmount;
            } else if (booking.payoutStatus === BOOKING_STATUS.PAYOUT_DONE) {
                paidOut += ownerAmount;
            }
        });
        ...
        const completedRequests = payoutRequestsSnapshot.docs.filter(doc => 
            doc.data().status === 'paid' || doc.data().status === 'approved'
        ).length;
        ...
        container.innerHTML = `
            ...
            <div class="payout-summary-grid">
                <div class="payout-summary-card">
                    ...
                    <div class="payout-summary-info">
                        <span class="payout-summary-label">Pending Payout</span>
                        <span class="payout-summary-amount">${formatCurrency(pendingPayout)}</span>
                    </div>
                </div>
                <div class="payout-summary-card">
                    ...
                    <div class="payout-summary-info">
                        ...
                        <span class="payout-summary-amount">${formatCurrency(paidOut)}</span>
                    </div>
                </div>
                <div class="payout-summary-card">
            ...
            <div class="available-balance-card">
                ...
                <div class="available-balance-amount">${formatCurrency(pendingPayout)}</div>
                <p class="available-balance-note">Platform commission (10%) already deducted</p>
                ${pendingPayout > 0 ? `
                    <button class="request-payout-btn-enhanced" onclick="showPayoutRequestModal(${pendingPayout})">
...
function showPayoutRequestModal(availableAmount) {
    ...
    content.innerHTML = `
        ...
        <div class="form-group">
            ...
            <input type="number" id="payout-amount" class="modal-input" value="${availableAmount}" min="500" max="${availableAmount}" step="1">
            <small>Minimum payout: ₹500</small>
        </div>
...
async function handleCreateTournament(e) {
    ...
    console.log('Create tournament form submitted'); // Debug log
    ...
    const prizeFirst = document.getElementById('prize-first')?.value;
    const prizeSecond = document.getElementById('prize-second')?.value;
    const prizeThird = document.getElementById('prize-third')?.value;
    
    let prizeDistribution = null;
    if (prizeFirst || prizeSecond || prizeThird) {
        prizeDistribution = {
            first: prizeFirst ? parseFloat(prizeFirst) : null,
            second: prizeSecond ? parseFloat(prizeSecond) : null,
            third: prizeThird ? parseFloat(prizeThird) : null
        };
    }
    
    // Validate all required fields
    if (!tournamentName || !sportType || !groundId || !format || !startDate || !endDate || 
        !startTime || !endTime || !teamSize || !maxTeams || !rules || !contactInfo) {
        showToast('Please fill all required fields', 'error');
        console.log('Missing fields:', { tournamentName, sportType, groundId, format, startDate, endDate, startTime, endTime, teamSize, maxTeams, rules, contactInfo });
        return;
    }
...
function showAddGroundModal() {
    // No payment check - all owners can add grounds for free
    // Reset form
    const form = document.getElementById('add-ground-form');
    if (form) form.reset();
...
function initializeImageUpload() {
    ...
    const newSelectPhotosBtn = selectPhotosBtn ? selectPhotosBtn.cloneNode(true) : null;
    uploadArea.parentNode.replaceChild(newUploadArea, uploadArea);
    if (selectPhotosBtn && newSelectPhotosBtn) {
    ...
    if (newSelectPhotosBtn) {
        newSelectPhotosBtn.addEventListener('click', () => fileInput.click());
    }
