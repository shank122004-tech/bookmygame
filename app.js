...
// Add this near the top of app.js after the Firebase initialization
    ...
    if (!window.crypto.subtle) {
        window.crypto.subtle = {
            digest: async function(algorithm, data) {
                // Fallback - in production, use server-side
                console.warn('Using fallback crypto - not secure for production');
...
const COLLECTIONS = {
    USERS: 'users',
    OWNERS: 'owners',
    PENDING_TOURNAMENT_REGISTRATIONS: 'pending_tournament_registrations',
    ADMINS: 'admins',
    VENUES: 'venues',
    GROUNDS: 'grounds',
    SLOTS: 'slots',
...
let listeners = {};
// Add these global variables for filtering
...
async function safeFirestoreOperation(operation, fallbackValue = null) {
    try {
        ...
        if (error.code === 'permission-denied') {
            ...
            showToast('You don\'t have permission to perform this action. Please contact support.', 'warning');
...
async function canAddGround() {
    ...
    try {
        ...
        const ownerDoc = await safeFirestoreOperation(
            () => db.collection(COLLECTIONS.OWNERS).doc(currentUser.uid).get(),
            null
        );
        ...
        // If ownerType is missing or invalid
        if (!owner.ownerType) {
        ...
        if (error.message && error.message.includes('permission-denied')) {
            showToast('Permission error. Please refresh and try again.', 'error');
        } else {
...
// ==================== HANDLE ADD GROUND (WITH FIREBASE STORAGE) ====================
...
async function handleAddGround(e) {
    ...
    if (uploadStatus) uploadStatus.textContent = 'Uploading images...';
    ...
    try {
        ...
        // Update the upload loop to compress images
if (groundImages.length > 0) {
    for (let i = 0; i < groundImages.length; i++) {
        ...
        const url = await uploadFile(fileToUpload, `grounds/${currentUser.uid}`);
        imageUrls.push(url);
        
        uploaded++;
        const progress = (uploaded / groundImages.length) * 100;
        if (progressFill) progressFill.style.width = `${progress}%`;
        if (uploadStatus) uploadStatus.textContent = `Uploaded ${uploaded} of ${groundImages.length} photos...`;
    }
}
        
        // Prepare ground data with image URLs
        const groundData = {
            ...
            images: imageUrls, // Store URLs from Firebase Storage
            rating: 0,
            totalReviews: 0,
            status: 'active',
            isVerified: false,
        ...
        const message = imageUrls.length === 0 ? 
            ...
            `Ground added successfully with ${imageUrls.length} photo(s)!`;
        ...
        let errorMessage = 'Error adding ground. ';
        if (error.message.includes('CORS')) {
            errorMessage = 'CORS error: Please configure Firebase Storage CORS. Contact support.';
...
// ==================== RESET ADD GROUND FORM ====================

function resetAddGroundForm() {
...
async function loadProfilePage() {
    ...
    if (currentUser.role === 'owner') {
     ...
     // Initialize invite friends section for users
    if (currentUser.role === 'user') {
        initializeInviteFriends();
...
async function filterBySport(sport) {
    showLoading(`Loading ${sport} venues and grounds...`);
    
    try {
        const container = document.getElementById('nearby-venues');
        
        // Get both venues and grounds
        ...
        venuesSnapshot.forEach(doc => {
            ...
            if (venue.sportType && venue.sportType.toLowerCase() === sport.toLowerCase()) {
                filteredVenues.push({ id: doc.id, type: 'venue', ...venue });
            }
        });
        
        // Filter grounds by sport
        let filteredGrounds = [];
        for (const doc of groundsSnapshot.docs) {
            const ground = doc.data();
            if (ground.sportType && ground.sportType.toLowerCase() === sport.toLowerCase()) {
                // Get owner info
                const ownerDoc = await db.collection(COLLECTIONS.OWNERS).doc(ground.ownerId).get();
                let ownerType = 'venue_owner';
                if (ownerDoc.exists) {
                    ownerType = ownerDoc.data().ownerType;
                }
                filteredGrounds.push({ 
                    id: doc.id, 
                    type: 'ground', 
                    ...ground,
                    ownerType: ownerType
        ...
        // Combine all items
        let allItems = [...filteredVenues, ...filteredGrounds];
        
        // Add distance calculation if user location available
        if (userLocation) {
            allItems = allItems.map(item => {
                let distance = Infinity;
                if (item.type === 'venue' && item.location) {
                    ...
                    distance = 5; // Default 5km for grounds
                }
        ...
        container.innerHTML = allItems.map(item => {
            ...
            if (item.type === 'venue') {
                return `
                    <div class="venue-card" data-venue-id="${item.id}" data-type="venue">
                        <img src="${item.images?.[0] || 'https://via.placeholder.com/120'}" 
                             alt="${item.venueName}" 
                             class="venue-image"
                             onerror="this.src='https://via.placeholder.com/120'">
                        <div class="venue-info">
                            <h3>${escapeHtml(item.venueName)} ${verifiedBadge}</h3>
                            <div class="venue-sport">${item.sportType}</div>
                            <div class="venue-rating">
                                <i class="fas fa-star"></i> ${(item.rating || 0).toFixed(1)}
                                <span style="color: var(--gray-500);">(${item.totalReviews || 0})</span>
                            </div>
                            ${distanceText ? `<div class="venue-distance"><i class="fas fa-location-dot"></i> ${distanceText}</div>` : ''}
                            <div class="venue-type-badge" style="margin-top: 8px;">
                                <span class="badge venue-owner-badge" style="background: var(--primary-50); color: var(--primary); padding: 2px 8px; border-radius: 12px; font-size: 10px;">
                                    <i class="fas fa-building"></i> Sports Facility
                                </span>
                            </div>
                        </div>
                    </div>
                ...
                const groundAddress = item.groundAddress || 'Address not specified';
                return `
                    <div class="venue-card" data-ground-id="${item.id}" data-type="ground">
                        <img src="${item.images?.[0] || 'https://via.placeholder.com/120'}" 
                             alt="${item.groundName}" 
                             class="venue-image"
                             onerror="this.src='https://via.placeholder.com/120'">
                        <div class="venue-info">
                            <h3>${escapeHtml(item.groundName)} ${verifiedBadge}</h3>
                            <div class="venue-sport">${item.sportType}</div>
                            <div class="venue-rating">
                                <i class="fas fa-star"></i> ${(item.rating || 0).toFixed(1)}
                                <span style="color: var(--gray-500);">(${item.totalReviews || 0})</span>
                            </div>
                            <div class="venue-address" style="font-size: var(--font-xs); color: var(--gray-500);">
                                <i class="fas fa-map-pin"></i> ${escapeHtml(groundAddress)}
                            </div>
                            ${distanceText ? `<div class="venue-distance"><i class="fas fa-location-dot"></i> ${distanceText}</div>` : ''}
                            <div class="venue-type-badge" style="margin-top: 8px;">
                                <span class="badge plot-owner-badge" style="background: rgba(16, 185, 129, 0.1); color: var(--success); padding: 2px 8px; border-radius: 12px; font-size: 10px;">
                                    <i class="fas fa-tree"></i> Open Ground
                                </span>
                                <span class="price-badge" style="margin-left: 8px; background: var(--gray-100); color: var(--gray-700); padding: 2px 8px; border-radius: 12px; font-size: 10px;">
        ...
        // Add click handlers
        document.querySelectorAll('.venue-card[data-venue-id]').forEach(card => {
            card.addEventListener('click', () => viewVenue(card.dataset.venueId));
        });
        document.querySelectorAll('.venue-card[data-ground-id]').forEach(card => {
            card.addEventListener('click', () => viewGround(card.dataset.groundId));
        });
...
// ==================== INITIATE PHONEPE MATCH PAYMENT (FIXED) ====================
...
async function initiatePhonePeMatchPayment(matchId, match, amount, upiApp, transactionId, userName) {
    ...
    try {
        ...
        // Generate SHA256 hash - FIX: Use crypto API safely
        let hashHex;
        try {
            ...
            throw new Error('Unable to generate payment signature. Please try again.');
        }
        ...
        // PhonePe API URL (UAT for testing)
        ...
        try {
            ...
            console.error('Error removing pending participant:', dbError);
        }
    }
}
...
// ==================== VERIFY PHONEPE PAYMENT STATUS (FIXED) ====================
...
async function verifyPhonePePaymentStatus(transactionId) {
    try {
        ...
        try {
            ...
            console.error('Crypto error in verification:', cryptoError);
            return {
                success: false,
...
// ==================== LOAD NEARBY VENUES AND GROUNDS ====================
...
async function loadNearbyVenues() {
    ...
    try {
        showLoading('Loading nearby venues and grounds...');
        ...
        for (const doc of groundsSnapshot.docs) {
            ...
            if (ownerDoc.exists) {
                ownerType = ownerDoc.data().ownerType;
                ownerName = ownerDoc.data().ownerName;
            }
        ...
        if (userLocation) {
            allItems = allItems.map(item => {
                ...
                // Try to get location from venue or ground
                if (item.type === 'venue' && item.location) {
                    distance = calculateDistance(
                        userLocation.lat,
                        userLocation.lng,
                    ...
                    // For grounds, use a default distance or try to get from venue if available
                    distance = 5; // Default 5km for plot owner grounds
                }
        ...
        if (displayItems.length === 0) {
            container.innerHTML = `
                <div class="empty-state">
                    <i class="fas fa-map-marker-alt"></i>
                    <h3>No venues or grounds found nearby</h3>
                    <p>Be the first to list your ground!</p>
                    ${currentUser && currentUser.role === 'owner' ? 
                        '<button class="auth-btn" onclick="showAddGroundModal()">List Your Ground</button>' : 
        ...
        // Display items with proper styling
            ...
            if (item.type === 'venue') {
                ...
                return `
                    <div class="venue-card" data-venue-id="${item.id}" data-type="venue">
                        ...
                        <div class="venue-info">
                            <h3>${escapeHtml(item.venueName)} ${verifiedBadge}</h3>
                            <div class="venue-sport">${item.sportType || 'Multi-sport'}</div>
                            <div class="venue-rating">
                                <i class="fas fa-star"></i> ${(item.rating || 0).toFixed(1)}
                                <span style="color: var(--gray-500);">(${item.totalReviews || 0})</span>
                            </div>
                            <div class="venue-address" style="font-size: var(--font-xs); color: var(--gray-500);">
                                <i class="fas fa-map-marker-alt"></i> ${item.address || 'Address not available'}
                            </div>
                            ${distanceText ? `
                ...
                // Plot owner ground display
                const groundAddress = item.groundAddress || 'Address not specified';
                const groundImages = item.images || [];
                
                return `
                    <div class="venue-card" data-ground-id="${item.id}" data-type="ground">
                        <img src="${groundImages[0] || 'https://via.placeholder.com/120'}" 
                             alt="${item.groundName}" 
                             class="venue-image"
                             onerror="this.src='https://via.placeholder.com/120'">
                        <div class="venue-info">
        ...
        // Add click handlers for both venues and grounds
        document.querySelectorAll('.venue-card[data-venue-id]').forEach(card => {
        ...
        hideLoading();
        console.error('Error loading venues and grounds:', error);
        container.innerHTML = `
            <div class="error-state">
                <i class="fas fa-exclamation-circle"></i>
                <p>Failed to load nearby venues and grounds</p>
                <button class="retry-btn" onclick="loadNearbyVenues()">Retry</button>
            </div>
        `;
    }
}
...
async function viewGround(groundId) {
    ...
    try {
        ...
        if (ownerDoc.exists) {
            ...
            isPlotOwner = owner.ownerType === OWNER_TYPES.PLOT_OWNER;
            ownerInfo = `
                <i class="fas fa-user"></i>
                <span>Owner: ${owner.ownerName || 'Unknown'} | Phone: ${owner.phone || 'Not available'}</span>
                ${isPlotOwner ? '<span class="plot-owner-badge"><i class="fas fa-tree"></i> Open Ground Owner</span>' : ''}
            `;
        }
        ...
        if (!isPlotOwner) {
            ...
            if (!venueSnapshot.empty) {
                ...
                venueInfo = currentVenue;
            } else {
                // If no venue found, use ground address only
                groundAddress = currentGround.groundAddress || 'Address not available';
                currentVenue = null;
                sessionStorage.removeItem('currentVenue');
            }
        } else {
            // For plot owners, use the ground's own address
            groundAddress = currentGround.groundAddress || 'Location available upon booking';
            currentVenue = null;
            sessionStorage.removeItem('currentVenue');
        }
        ...
        const plotOwnerBadge = document.getElementById('plot-owner-badge');
        ...
        // Show address with appropriate icon
        if (isPlotOwner) {
            groundAddressEl.innerHTML = `
                <i class="fas fa-tree"></i>
                <span>${escapeHtml(groundAddress)}</span>
                <span class="plot-owner-tag" style="margin-left: auto; font-size: var(--font-xs); color: var(--success);">
                    <i class="fas fa-check-circle"></i> Open Ground
                </span>
            `;
        } else {
            groundAddressEl.innerHTML = `
        ...
        // Show/hide plot owner badge in header
        if (plotOwnerBadge) {
            if (isPlotOwner) {
                plotOwnerBadge.style.display = 'inline-flex';
                plotOwnerBadge.innerHTML = '<i class="fas fa-tree"></i> Open Ground';
            } else {
                plotOwnerBadge.style.display = 'none';
            }
        }
        
        // Store in session storage that this is a plot owner ground
...
function selectSlot(slot) {
    ...
    // Check if this is a plot owner ground
    const isPlotOwner = sessionStorage.getItem('isPlotOwnerGround') === 'true';
    ...
    const venueInfoSection = document.getElementById('venue-info-section');
    const groundTypeBadge = document.getElementById('ground-type-badge');
    ...
    // Add plot owner info to booking page
    if (isPlotOwner) {
        if (groundTypeBadge) {
            groundTypeBadge.innerHTML = '<i class="fas fa-tree"></i> Open Ground Booking';
            groundTypeBadge.style.display = 'block';
        }
        if (venueInfoSection) {
            venueInfoSection.innerHTML = `
                <div class="info-note" style="background: rgba(16, 185, 129, 0.1); margin-bottom: var(--space-lg);">
                    <i class="fas fa-info-circle"></i>
                    <span>This is an open ground. Please contact owner for exact location details after booking.</span>
                </div>
            `;
        }
    } else {
        if (groundTypeBadge) groundTypeBadge.style.display = 'none';
        if (venueInfoSection) venueInfoSection.innerHTML = '';
    }
 ...
 */
// ==================== FIXED PHONEPE PAYMENT WITH SAFE CRYPTO ====================
async function initiatePhonePePayment(bookingDetails) {
    ...
    try {
        ...
        // FIX: Safe crypto check with fallback
        let hashHex;
        ...
        if (typeof window !== 'undefined' && window.crypto && window.crypto.subtle) {
            try {
                ...
                const hashArray = Array.from(new Uint8Array(hashBuffer));
                hashHex = hashArray.map(b => b.toString(16).padStart(2, '0')).join('');
                console.log('Crypto hash generated successfully');
            } catch (cryptoError) {
                console.error('Crypto error:', cryptoError);
                // Fallback to simple hash for demonstration only
                // In production, you should use a server-side API for payment
                hashHex = generateFallbackHash(stringToSign);
...
function generateFallbackHash(string) {
    ...
    // DO NOT use this for production - use proper crypto or server-side
    let hash = 0;
...
async function handleUPIPayment(upiApp) {
    ...
    if (!currentGround || !currentVenue) {
        ...
        const isPlotOwnerSaved = sessionStorage.getItem('isPlotOwnerGround') === 'true';
        ...
        if (savedGround) {
            ...
            // For plot owners, we don't need venue info
            if (isPlotOwnerSaved) {
                currentVenue = null;
                console.log('Recovered plot owner ground from session storage');
            } else if (savedVenue) {
                currentVenue = JSON.parse(savedVenue);
                console.log('Recovered ground and venue from session storage');
            } else {
                // Try to get venue info if it's not a plot owner
                if (!isPlotOwnerSaved) {
                    const venueSnapshot = await db.collection(COLLECTIONS.VENUES)
                        .where('ownerId', '==', currentGround.ownerId)
                        .limit(1)
                        .get();
                    if (!venueSnapshot.empty) {
                        currentVenue = { id: venueSnapshot.docs[0].id, ...venueSnapshot.docs[0].data() };
                        sessionStorage.setItem('currentVenue', JSON.stringify(currentVenue));
                    }
                }
            }
    ...
    if (isPlotOwner) {
        // For plot owners, use ground address
        venueName = 'Open Ground';
        venueAddress = currentGround.groundAddress || 'Location details will be shared after booking';
        fullAddress = venueAddress;
    } else if (currentVenue) {
        venueAddress = currentVenue.address || '';
        venueName = currentVenue.venueName || 'Unknown Venue';
        const groundAddress = currentGround.groundAddress || '';
        fullAddress = groundAddress ? `${venueAddress} - ${groundAddress}` : venueAddress;
    } else {
        venueName = currentGround.groundName || 'Ground';
        venueAddress = currentGround.groundAddress || 'Address not specified';
        fullAddress = venueAddress;
    }
    ...
    // Store booking details in session for recovery
...
// Replace your processTournamentRegistration function with this corrected version

// ==================== PROCESS TOURNAMENT REGISTRATION (FIXED) ====================
...
async function processTournamentRegistration(tournamentId) {
    ...
    try {
        ...
        const existingPending = await db.collection(COLLECTIONS.PENDING_TOURNAMENT_REGISTRATIONS)
            .where('tournamentId', '==', tournamentId)
            .where('userId', '==', currentUser.uid)
            .where('status', '==', 'pending')
            .get();
        ...
        if (!existingPending.empty) {
            const pendingReg = existingPending.docs[0].data();
            const confirmResume = confirm('You have a pending registration. Would you like to continue with that payment?');
            if (confirmResume) {
                hideLoading();
                if (submitBtn) {
                ...
                // Call showTournamentPayment with the existing registration
                showTournamentPayment(tournamentData, pendingReg.registrationId, pendingReg.teamName);
                return;
            } else {
                await existingPending.docs[0].ref.delete();
            }
        }
        
        // Check if user is already successfully registered
        ...
        // IMPORTANT: Store ONLY in PENDING collection - NOT in TOURNAMENT_REGISTRATIONS
        // This ensures registration is NOT confirmed until payment is verified
        ...
        await db.collection(COLLECTIONS.PENDING_TOURNAMENT_REGISTRATIONS).add(pendingRegistrationData);
        ...
        // Show payment page - NOT success modal
...
// ==================== UPDATED SHOW TOURNAMENT PAYMENT ====================

// ==================== FIXED SHOW TOURNAMENT PAYMENT ====================

// Replace the showTournamentPayment function with this corrected version

// ==================== FIXED SHOW TOURNAMENT PAYMENT ====================

// ==================== FIXED SHOW TOURNAMENT PAYMENT ====================
function showTournamentPayment(tournament, registrationId, teamName) {
    console.log('showTournamentPayment called with:', { tournament, registrationId, teamName });
    
    // Ensure tournament has proper id - try multiple ways to get the ID
    let tournamentData = tournament;
    let tournamentId = null;
    
    // Helper function to extract ID from various formats
    function getTournamentId(t) {
        if (!t) return null;
        if (t.id) return t.id;
        if (t.tournamentId) return t.tournamentId;
        if (t._document && t._document.key && t._document.key.path) {
            const segments = t._document.key.path.segments;
    ...
    tournamentId = getTournamentId(tournamentData);
    
    // If still no ID, try from currentTournament
    if (!tournamentId && currentTournament) {
        tournamentId = getTournamentId(currentTournament);
        if (tournamentId) {
            tournamentData = currentTournament;
            console.log('Using currentTournament with ID:', tournamentId);
        }
    }
    
    // If still no ID, try from window.currentTournamentPayment
    if (!tournamentId && window.currentTournamentPayment && window.currentTournamentPayment.tournament) {
        tournamentId = getTournamentId(window.currentTournamentPayment.tournament);
        if (tournamentId) {
            tournamentData = window.currentTournamentPayment.tournament;
            console.log('Using window.currentTournamentPayment.tournament with ID:', tournamentId);
        }
    }
    
    // If we have registrationId but no tournament data, try to get from pending registration
    if (!tournamentId && registrationId) {
        const pendingRegStr = sessionStorage.getItem('pendingTournamentRegistration');
        if (pendingRegStr) {
            try {
                const pendingReg = JSON.parse(pendingRegStr);
                if (pendingReg.tournamentId) {
                    tournamentId = pendingReg.tournamentId;
                    console.log('Got tournamentId from pending registration:', tournamentId);
                    // We still need tournament data, but we'll fetch it in the payment function
                    tournamentData = { id: tournamentId, tournamentName: pendingReg.tournamentName };
                }
            } catch (e) {
    ...
    // If still no tournamentId, show error
    if (!tournamentId) {
        console.error('Tournament ID not found. Tournament object:', tournamentData);
        console.error('CurrentTournament:', currentTournament);
        console.error('window.currentTournamentPayment:', window.currentTournamentPayment);
        console.error('Session storage pendingTournamentRegistration:', sessionStorage.getItem('pendingTournamentRegistration'));
        showToast('Error: Tournament information missing. Please go back and try again.', 'error');
        return;
    }
    
    // If we have tournamentId but no tournamentData, set a minimal object
    if (!tournamentData || typeof tournamentData !== 'object') {
        tournamentData = { id: tournamentId };
    } else if (!tournamentData.id && tournamentId) {
        tournamentData.id = tournamentId;
    }
    
    // Ensure tournamentData has the required fields
    tournamentData.id = tournamentId;
    
    const amount = tournamentData.entryFee || 0;
    
    const startDate = tournamentData.startDate ? new Date(tournamentData.startDate) : new Date();
    const formattedDate = startDate.toLocaleDateString('en-IN', {
        day: 'numeric',
        month: 'long',
        year: 'numeric'
    ...
    if (!container) {
        console.error('Tournament payment container not found');
        return;
    }
    
    // Store tournament data globally for payment function to access
    window.currentTournamentPayment = {
    ...
    // Also store in currentTournament for backup
        ...
        <div class="tournament-payment-container">
            <div class="payment-card">
                    ...
                    <div class="summary-details">
                        <div class="summary-detail-item">
                            ...
                            <div class="summary-detail-content">
                                ...
                                <span class="summary-detail-value">${escapeHtml(tournamentData.tournamentName || 'Tournament')}</span>
                            </div>
                        </div>
                        <div class="summary-detail-item">
                            <i class="fas fa-calendar-alt"></i>
                        ...
                        <div class="summary-detail-item">
                            ...
                            <div class="summary-detail-content">
                                ...
                                <span class="summary-detail-value">${escapeHtml(tournamentData.venueName || 'TBD')}</span>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="upi-payment-section">
                    <div class="upi-payment-title">
                        <i class="fas fa-mobile-alt"></i>
                        <h3>Select Payment Method</h3>
                    </div>
                    <div class="upi-payment-subtitle">
                        Choose your preferred payment option
                    </div>
                    
                    <div class="upi-apps-grid-modern">
                        <div class="upi-app-modern" data-payment-type="paytm">
                            <i class="fas fa-mobile-alt" style="font-size: 2rem; color: #1b2e6c;"></i>
                            <span>Paytm</span>
                        </div>
                        <div class="upi-app-modern" data-payment-type="upi">
                ...
                <div style="margin: var(--space-xl);">
                    <button class="paytm-payment-btn" id="paytm-pay-btn" 
                            style="width: 100%; padding: var(--space-lg); background: linear-gradient(135deg, #1b2e6c, #0b1a3e); color: white; border: none; border-radius: var(--radius); font-size: var(--font-lg); font-weight: 600; cursor: pointer; display: flex; align-items: center; justify-content: center; gap: var(--space-md); transition: all var(--transition);">
                        <i class="fas fa-lock"></i>
                        Pay ${formatCurrency(amount)} via Paytm
                        <i class="fas fa-arrow-right"></i>
                    </button>
                    <p class="payment-note-modern" style="margin-top: var(--space-md); text-align: center;">
                        <i class="fas fa-shield-alt"></i>
                        Secured by Paytm | PCI DSS Compliant
                    </p>
                </div>
    ...
    // Add click handler for Paytm payment button
    ...
    if (payBtn) {
        ...
        newPayBtn.addEventListener('click', async () => {
            const selectedApp = document.querySelector('.upi-app-modern.selected');
            const paymentType = selectedApp ? selectedApp.dataset.paymentType : 'paytm';
    ...
    // Add payment type selection handlers
    document.querySelectorAll('.upi-app-modern').forEach(app => {
        const newApp = app.cloneNode(true);
        app.parentNode.replaceChild(newApp, app);
        
        newApp.addEventListener('click', function() {
            document.querySelectorAll('.upi-app-modern').forEach(a => a.classList.remove('selected'));
            ...
            if (payBtn) {
                ...
                payBtn.innerHTML = `<i class="fas fa-lock"></i> Pay ${formatCurrency(amount)} via ${paymentTypeText[paymentType]}<i class="fas fa-arrow-right"></i>`;
                payBtn.dataset.paymentType = paymentType;
            }
        });
    });
...
async function deleteGround(groundId, groundName) {
    // Show confirmation dialog
    const confirmed = await showDeleteConfirmationModal(
        'Delete Ground',
        `Are you sure you want to delete "${groundName}"?`,
        'This action cannot be undone. All associated bookings and slots will be permanently removed.'
    );
    
    if (!confirmed) return;
    
    showLoading('Deleting ground...');
    
    try {
        const groundRef = db.collection(COLLECTIONS.GROUNDS).doc(groundId);
        const groundDoc = await groundRef.get();
        ...
        // Verify the current user owns this ground
        if (ground.ownerId !== currentUser.uid) {
            throw new Error('You do not have permission to delete this ground');
        }
        ...
        // Check if there are any future confirmed bookings
        const today = new Date().toISOString().split('T')[0];
        const futureBookings = bookingsSnapshot.docs.filter(doc => {
            const booking = doc.data();
            return booking.date >= today && booking.bookingStatus === BOOKING_STATUS.CONFIRMED;
        });
        
        if (futureBookings.length > 0) {
            throw new Error(`Cannot delete ground with ${futureBookings.length} upcoming confirmed bookings. Please cancel them first or contact support.`);
        }
        ...
        // Delete all bookings (can only delete past or cancelled ones)
        bookingsSnapshot.forEach(doc => {
            batch.delete(doc.ref);
        });
        
        // Delete the ground
        batch.delete(groundRef);
        ...
        hideLoading();
        showToast(`Ground "${groundName}" deleted successfully!`, 'success');
        ...
        showToast('Error deleting ground: ' + error.message, 'error');
    }
}
...
async function deleteVenue(venueId, venueName) {
    ...
    const confirmed = await showDeleteConfirmationModal(
        ...
        'This action cannot be undone. All associated grounds, slots, and bookings will be permanently removed.'
    );
    
    if (!confirmed) return;
    
    showLoading('Deleting venue...');
    ...
    try {
        ...
        // Get all grounds for this venue
        const groundsSnapshot = await db.collection(COLLECTIONS.GROUNDS)
            .where('ownerId', '==', currentUser.uid)
            .get();
        
        // Check if there are any future confirmed bookings across all grounds
        const today = new Date().toISOString().split('T')[0];
        ...
        for (const doc of groundsSnapshot.docs) {
            ...
            if (!bookingsSnapshot.empty) {
                ...
                futureBookingCount += bookingsSnapshot.size;
            }
        }
        
        if (hasFutureBookings) {
            throw new Error(`Cannot delete venue with ${futureBookingCount} upcoming confirmed bookings across its grounds. Please cancel them first or contact support.`);
        }
        
        // Start a batch operation for all deletions
        const batch = db.batch();
        
        // Delete all grounds and their associated data
        for (const doc of groundsSnapshot.docs) {
            const groundId = doc.id;
            
            // Get slots for this ground
            const slotsSnapshot = await db.collection(COLLECTIONS.SLOTS)
                .where('groundId', '==', groundId)
                .get();
        ...
        // Update owner's grounds count to 0
        await db.collection(COLLECTIONS.OWNERS).doc(currentUser.uid).update({
            groundsCount: 0,
            updatedAt: firebase.firestore.FieldValue.serverTimestamp()
        });
        ...
        hideLoading();
        showToast(`Venue "${venueName}" and all its grounds deleted successfully!`, 'success');
        ...
        showToast('Error deleting venue: ' + error.message, 'error');
    }
}
...
async function loadOwnerGrounds(container) {
    ...
    try {
        ...
        const hasVenue = !venueSnapshot.empty;
        const venueId = hasVenue ? venueSnapshot.docs[0].id : null;
        const venueName = hasVenue ? venueSnapshot.docs[0].data().venueName : null;
        
        html += `
            <div style="display: flex; gap: var(--space-md); margin-bottom: var(--space-xl); flex-wrap: wrap;">
                <button class="auth-btn" id="add-ground-btn" style="flex: 1;">
                    <i class="fas fa-plus"></i> Add New Ground
                </button>
                ${hasVenue ? `
                    <button class="delete-venue-btn" id="delete-venue-btn" style="flex: 1; background: var(--danger); color: white;">
        ...
        if (snapshot.empty) {
            ...
            html += '<div class="grounds-management-grid">';
            snapshot.forEach(doc => {
                const ground = doc.data();
                const verifiedBadge = ground.isVerified ? 
                    '<span class="verified-badge"><i class="fas fa-check-circle"></i> Verified</span>' : '';
                
                // Check if there are future bookings for this ground
                const today = new Date().toISOString().split('T')[0];
                let hasFutureBookings = false;
                ...
                html += `
                    <div class="ground-management-card" data-ground-id="${doc.id}">
                        ...
                        <div class="ground-actions" style="margin-top: var(--space-md);">
                            ...
                            <button class="delete-ground-btn" data-ground-id="${doc.id}" data-ground-name="${escapeHtml(ground.groundName)}" style="background: var(--danger); color: white;">
                                <i class="fas fa-trash-alt"></i> Delete Ground
                            </button>
                        </div>
                    </div>
                `;
        ...
        // Add event listener for Delete Venue button
        const deleteVenueBtn = document.getElementById('delete-venue-btn');
        if (deleteVenueBtn && venueId) {
            const newDeleteVenueBtn = deleteVenueBtn.cloneNode(true);
            deleteVenueBtn.parentNode.replaceChild(newDeleteVenueBtn, deleteVenueBtn);
            newDeleteVenueBtn.addEventListener('click', () => {
        ...
        // Add event listeners for Delete Ground buttons
        document.querySelectorAll('.delete-ground-btn').forEach(btn => {
            const newBtn = btn.cloneNode(true);
            btn.parentNode.replaceChild(newBtn, btn);
            newBtn.addEventListener('click', () => {
                deleteGround(newBtn.dataset.groundId, newBtn.dataset.groundName);
            });
        });
...
// Enhanced loadOwnerGrounds function
async function loadOwnerGrounds(container) {
    showLoading('Loading grounds...');
    ...
    try {
        ...
        const venueId = hasVenue ? venueSnapshot.docs[0].id : null;
        const venueName = hasVenue ? venueSnapshot.docs[0].data().venueName : null;
        const ownerType = currentUser.ownerType;
        const isPlotOwner = ownerType === OWNER_TYPES.PLOT_OWNER;
        
        let html = `
            <div class="grounds-dashboard-header">
                <div class="header-left">
                    <h3><i class="fas fa-futbol"></i> Your Grounds</h3>
                    <p class="grounds-count">${snapshot.size} ground${snapshot.size !== 1 ? 's' : ''} listed</p>
                </div>
                <div class="header-right">
                    <button class="add-ground-btn" id="add-ground-btn">
                        <i class="fas fa-plus"></i>
                        Add New Ground
                    </button>
                    ${hasVenue && isPlotOwner ? `
                        <button class="delete-venue-btn" id="delete-venue-btn">
                            <i class="fas fa-trash-alt"></i>
                            Delete Venue
                        </button>
                    ` : ''}
        ...
        if (snapshot.empty) {
            html += `
                <div class="empty-grounds-state">
                    <div class="empty-icon">
                        <i class="fas fa-futbol"></i>
                    </div>
                    <h3>No Grounds Listed Yet</h3>
                    <p>Start earning by listing your first ground!</p>
                    <button class="add-ground-btn-primary" onclick="showAddGroundModal()">
            ...
            html += '<div class="grounds-grid-modern">';
            for (const doc of snapshot.docs) {
                const ground = doc.data();
                const verifiedBadge = ground.isVerified ? 
                    '<span class="verified-badge"><i class="fas fa-check-circle"></i> Verified</span>' : '';
                
                // Get booking stats for this ground
                const today = new Date().toISOString().split('T')[0];
                const bookingsToday = await db.collection(COLLECTIONS.BOOKINGS)
                    .where('groundId', '==', doc.id)
                    .where('date', '==', today)
                    .where('bookingStatus', '==', BOOKING_STATUS.CONFIRMED)
                    .get();
                
                const upcomingBookings = await db.collection(COLLECTIONS.BOOKINGS)
                    .where('groundId', '==', doc.id)
                    .where('date', '>=', today)
                    .where('bookingStatus', '==', BOOKING_STATUS.CONFIRMED)
                    .limit(3)
                ...
                const hasBookings = upcomingBookings.size > 0;
                
                html += `
                    <div class="ground-card-modern" data-ground-id="${doc.id}">
                        <div class="ground-card-header-modern">
                            <div class="ground-info">
                                <div class="ground-image">
                                    <img src="${ground.images?.[0] || 'https://via.placeholder.com/60'}" 
                                         alt="${escapeHtml(ground.groundName)}"
                                         onerror="this.src='https://via.placeholder.com/60'">
                                </div>
                                <div class="ground-details">
                                    <h4>${escapeHtml(ground.groundName)} ${verifiedBadge}</h4>
                                    <div class="ground-meta">
                                        <span class="sport-tag">${escapeHtml(ground.sportType || 'Multi-sport')}</span>
                                        <span class="price-tag">${formatCurrency(ground.pricePerHour)}/hr</span>
                                    </div>
                                </div>
                            </div>
                            <div class="ground-stats">
                                <div class="stat-badge">
                                    <i class="fas fa-calendar-check"></i>
                                    <span>${bookingsToday.size} Today</span>
                                </div>
                                <div class="stat-badge rating">
                                    <i class="fas fa-star"></i>
                                    <span>${(ground.rating || 0).toFixed(1)}</span>
                                </div>
                            </div>
                        </div>
                        
                        <div class="ground-card-body">
                            <div class="ground-address">
                                <i class="fas fa-map-pin"></i>
                                <span>${escapeHtml(ground.groundAddress || 'Address not specified')}</span>
                            </div>
                            ...
                            ${hasBookings ? `
                                <div class="upcoming-bookings">
                                    ...
                                    <div class="booking-list">
                                        ${upcomingBookings.docs.map(bookingDoc => {
                                            const booking = bookingDoc.data();
                                            return `
                                                <div class="booking-mini">
                                                    <span class="booking-time">${booking.slotTime}</span>
                                                    <span class="booking-user">${escapeHtml(booking.userName || 'Player')}</span>
                                                </div>
                                            `;
                                        }).join('')}
                                    </div>
                        ...
                        <div class="ground-card-actions">
                            <button class="action-btn slots" data-ground-id="${doc.id}" data-ground-name="${escapeHtml(ground.groundName)}">
                                <i class="fas fa-clock"></i> Manage Slots
                            </button>
                            <button class="action-btn close-day" data-ground-id="${doc.id}">
                                <i class="fas fa-ban"></i> Close Day
                            </button>
                            <button class="action-btn view" data-ground-id="${doc.id}">
                                <i class="fas fa-eye"></i> View Details
                            </button>
                            <button class="action-btn delete" data-ground-id="${doc.id}" data-ground-name="${escapeHtml(ground.groundName)}">
        ...
        if (addBtn) {
            const newAddBtn = addBtn.cloneNode(true);
            addBtn.parentNode.replaceChild(newAddBtn, addBtn);
            newAddBtn.addEventListener('click', () => showAddGroundModal());
        }
        
        if (hasVenue && isPlotOwner) {
            const deleteVenueBtn = document.getElementById('delete-venue-btn');
            if (deleteVenueBtn) {
                const newDeleteVenueBtn = deleteVenueBtn.cloneNode(true);
                deleteVenueBtn.parentNode.replaceChild(newDeleteVenueBtn, deleteVenueBtn);
                newDeleteVenueBtn.addEventListener('click', () => deleteVenue(venueId, venueName));
            }
        }
        
        // Attach ground action handlers
        document.querySelectorAll('.action-btn.slots').forEach(btn => {
            const newBtn = btn.cloneNode(true);
            btn.parentNode.replaceChild(newBtn, btn);
            newBtn.addEventListener('click', () => manageSlots(newBtn.dataset.groundId, newBtn.dataset.groundName));
        });
        
        document.querySelectorAll('.action-btn.close-day').forEach(btn => {
            const newBtn = btn.cloneNode(true);
            btn.parentNode.replaceChild(newBtn, btn);
            newBtn.addEventListener('click', () => closeFullDay(newBtn.dataset.groundId));
        });
        
        document.querySelectorAll('.action-btn.view').forEach(btn => {
            const newBtn = btn.cloneNode(true);
            btn.parentNode.replaceChild(newBtn, btn);
            newBtn.addEventListener('click', () => viewGround(newBtn.dataset.groundId));
        });
        
        document.querySelectorAll('.action-btn.delete').forEach(btn => {
            const newBtn = btn.cloneNode(true);
            btn.parentNode.replaceChild(newBtn, btn);
            newBtn.addEventListener('click', () => deleteGround(newBtn.dataset.groundId, newBtn.dataset.groundName));
        });
...
// ==================== INITIALIZE IMAGE UPLOAD (BASE64) ====================
...
async function manageSlots(groundId, groundName) {
    ...
    try {
        ...
        // Create slot management modal if it doesn't exist
        let modal = document.getElementById('slot-management-modal');
        if (!modal) {
            const modalHtml = `
                <div id="slot-management-modal" class="modal">
                    <div class="modal-content modal-lg slot-management-modal">
                        <div class="modal-header">
                            <div class="modal-header-left">
                                ...
                                <div>
                                    <h3>Manage Slots</h3>
                                    <p class="modal-subtitle" id="slot-ground-name">Loading...</p>
                                </div>
                            </div>
                            <button class="close-btn" id="close-slot-management-modal">
                                <i class="fas fa-times"></i>
                            </button>
                        </div>
                        <div class="modal-body">
                            <div id="slot-management-content"></div>
                        </div>
                    </div>
                </div>
            `;
            ...
            // Add close button listener safely
            const closeBtn = document.getElementById('close-slot-management-modal');
            if (closeBtn) {
                closeBtn.addEventListener('click', () => {
                    closeModal('slot-management-modal');
                });
            }
        }
        
        // Update ground name - check if element exists first
        const groundNameElement = document.getElementById('slot-ground-name');
        if (groundNameElement) {
            groundNameElement.textContent = groundName;
        }
        
        // Load slots for the next 7 days
        await loadSlotsForDays(groundId, today, sevenDaysLater);
        
        // Get modal and add active class
        modal = document.getElementById('slot-management-modal');
        if (modal) {
            modal.classList.add('active');
        }
...
/**
 * Load slots for multiple days
 */
async function loadSlotsForDays(groundId, startDate, endDate) {
    ...
    try {
        ...
        // Create a map of slot statuses by date and time
        const slotStatusMap = {};
        slotsSnapshot.forEach(doc => {
            const slot = doc.data();
            const key = `${slot.date}_${slot.startTime}-${slot.endTime}`;
            slotStatusMap[key] = {
                status: slot.status,
                id: doc.id
            };
        ...
        // Build HTML for date selector and slots
        let html = `
            <div class="slot-management-header">
                <div class="date-navigation">
                    <button class="date-nav-btn" id="prev-week-btn">
                        <i class="fas fa-chevron-left"></i>
                    </button>
                    <div class="date-range" id="date-range-display"></div>
                    <button class="date-nav-btn" id="next-week-btn">
                        <i class="fas fa-chevron-right"></i>
                    </button>
                </div>
                <div class="slot-bulk-actions">
                    <button class="bulk-action-btn open-all" id="bulk-open-all">
                        <i class="fas fa-unlock-alt"></i> Open All
                    </button>
                    <button class="bulk-action-btn close-all" id="bulk-close-all">
                        <i class="fas fa-lock"></i> Close All
                    </button>
                    <button class="bulk-action-btn reset" id="bulk-reset">
                        <i class="fas fa-undo-alt"></i> Reset
                    </button>
                </div>
            </div>
            <div id="slots-week-view" class="slots-week-view"></div>
        `;
        ...
        function updateWeekView() {
            ...
            if (dateRangeDisplay) {
                ...
                const endStr = weekEnd.toLocaleDateString('en-IN', { day: 'numeric', month: 'short', year: 'numeric' });
                dateRangeDisplay.textContent = `${startStr} - ${endStr}`;
            }
            
            // Build week view table
            let weekHtml = `
                <div class="slots-week-table">
                    <div class="slots-week-header">
            ...
            weekHtml += `</div><div class="slots-week-body">`;
            ...
            for (const slot of defaultSlots) {
                ...
                for (let i = 0; i < 7; i++) {
                    ...
                    const slotData = slotStatusMap[key] || { status: 'available', id: null };
                    const statusClass = slotData.status;
                    const statusText = slotData.status === 'available' ? 'Available' : 
                                      slotData.status === 'closed' ? 'Closed' : 
                                      slotData.status === 'booked' ? 'Booked' : 'Pending';
                    
                    const isDisabled = slotData.status === 'booked' || slotData.status === 'pending';
                    
                    weekHtml += `
                        <div class="slot-cell ${statusClass}" 
                             data-ground-id="${groundId}"
                             data-date="${dateStr}"
                             data-slot="${slot}"
                             data-slot-id="${slotData.id || ''}"
                             data-start="${slot.split('-')[0]}"
                             data-end="${slot.split('-')[1]}">
                            ...
                            ${!isDisabled ? `
                                <div class="slot-actions">
                                    <button class="slot-action available" data-action="available">✓ Available</button>
                                    <button class="slot-action closed" data-action="closed">✗ Closed</button>
                                </div>
                            ` : `
                                <div class="slot-disabled-message">
                                    <i class="fas fa-ban"></i> ${statusText}
                                </div>
                            `}
                        </div>
                    `;
            ...
            const weekView = document.getElementById('slots-week-view');
            if (weekView) {
                weekView.innerHTML = weekHtml;
                attachSlotActionHandlers(groundId);
            }
        }
        ...
        // Inside loadSlotsForDays function, update the navigation handlers section:

// Add navigation handlers with null checks
const prevWeekBtn = document.getElementById('prev-week-btn');
const nextWeekBtn = document.getElementById('next-week-btn');

if (prevWeekBtn) {
    const newPrev = prevWeekBtn.cloneNode(true);
    prevWeekBtn.parentNode.replaceChild(newPrev, prevWeekBtn);
...
if (nextWeekBtn) {
    const newNext = nextWeekBtn.cloneNode(true);
    nextWeekBtn.parentNode.replaceChild(newNext, nextWeekBtn);
    newNext.addEventListener('click', () => {
        ...
        // Add bulk action handlers with null checks
const bulkOpenAll = document.getElementById('bulk-open-all');
const bulkCloseAll = document.getElementById('bulk-close-all');
const bulkReset = document.getElementById('bulk-reset');

if (bulkOpenAll) {
    const newOpen = bulkOpenAll.cloneNode(true);
    bulkOpenAll.parentNode.replaceChild(newOpen, bulkOpenAll);
    newOpen.addEventListener('click', () => bulkUpdateSlots(groundId, 'available'));
}

if (bulkCloseAll) {
    const newClose = bulkCloseAll.cloneNode(true);
    bulkCloseAll.parentNode.replaceChild(newClose, bulkCloseAll);
    newClose.addEventListener('click', () => bulkUpdateSlots(groundId, 'closed'));
}

if (bulkReset) {
    const newReset = bulkReset.cloneNode(true);
    bulkReset.parentNode.replaceChild(newReset, bulkReset);
    newReset.addEventListener('click', () => bulkUpdateSlots(groundId, 'available'));
}
        
    } catch (error) {
        console.error('Error loading slots for days:', error);
        container.innerHTML = `<p class="text-center">Error loading slots: ${error.message}</p>`;
    }
}

/**
 * Attach handlers to slot action buttons
 */
function attachSlotActionHandlers(groundId) {
    document.querySelectorAll('.slot-cell .slot-action').forEach(btn => {
        const newBtn = btn.cloneNode(true);
        btn.parentNode.replaceChild(newBtn, btn);
        ...
        newBtn.addEventListener('click', async (e) => { 
            ...
            const slotCell = newBtn.closest('.slot-cell');
            const date = slotCell.dataset.date;
            const slot = slotCell.dataset.slot;
            const slotId = slotCell.dataset.slotId;
            const startTime = slotCell.dataset.start;
            const endTime = slotCell.dataset.end;
            
            await updateSlotStatus(slotId, groundId, date, startTime, endTime, action);
        });
    });
}

/**
 * Update a single slot status
 */
async function updateSlotStatus(slotId, groundId, date, startTime, endTime, status) {
    ...
    try {
        ...
        hideLoading();
        showToast(`Slot ${status === 'available' ? 'opened' : 'closed'} successfully`, 'success');
        
        // Refresh the slot view
        const modal = document.getElementById('slot-management-modal');
        if (modal && modal.classList.contains('active')) {
            const groundName = document.getElementById('slot-ground-name').textContent;
            await manageSlots(groundId, groundName);
        }
...
/**
 * Bulk update slots for a ground
 */
async function bulkUpdateSlots(groundId, status) {
    const actionText = status === 'available' ? 'open all slots' : 'close all slots';
    const confirmed = confirm(`Are you sure you want to ${actionText} for the current week?`);
    if (!confirmed) return;
    
    showLoading(`Updating ${actionText}...`);
    ...
    try {
        ...
        slotsSnapshot.forEach(doc => {
            const slot = doc.data();
            // Only update if not booked or pending
        ...
        hideLoading();
        showToast(`Successfully ${actionText.replace('all', '')} for current week`, 'success');
        ...
        showToast('Error updating slots: ' + error.message, 'error');
    }
}
...
async function closeFullDay(groundId) {
    const date = prompt('Enter date (YYYY-MM-DD) to close all slots:', new Date().toISOString().split('T')[0]);
    if (!date) return;
    ...
    const confirmed = confirm(`Are you sure you want to close all slots for ${date}?`);
    if (!confirmed) return;
    ...
    try {
        ...
        for (const slot of defaultSlots) {
            ...
            if (snapshot.empty) {
                ...
                // Only update if not booked or pending
                if (slotData.status !== 'booked' && slotData.status !== 'pending') {
                    batch.update(snapshot.docs[0].ref, {
                        status: SLOT_STATUS.CLOSED,
                        updatedAt: firebase.firestore.FieldValue.serverTimestamp()
                    });
        ...
        hideLoading();
        showToast(`All slots closed for ${date}`, 'success');
        
        // Refresh the slot view if modal is open
        const modal = document.getElementById('slot-management-modal');
        if (modal && modal.classList.contains('active')) {
            const groundName = document.getElementById('slot-ground-name').textContent;
            await manageSlots(groundId, groundName);
        ...
        hideLoading();
        console.error('Error closing full day:', error);
        showToast('Error closing slots: ' + error.message, 'error');
    }
}
...
// ==================== ALL VENUES PAGE (INCLUDES GROUNDS) ====================

// ==================== ALL VENUES PAGE (INCLUDES BOTH VENUES AND GROUNDS) ====================

async function loadAllVenuesPage() {
    showLoading('Loading all venues and grounds...');
    ...
    try {
        ...
        if (!allVenuesPage) {
            ...
            allVenuesPage.innerHTML = `
                ...
                <div class="search-container" style="padding: var(--space-lg);">
                    <div class="search-bar" style="background: var(--gray-100);">
                        ...
                        <input type="text" placeholder="Search venues, grounds, sports..." id="all-venues-search" style="color: var(--gray-900);">
                    </div>
                </div>
                ...
                <div class="all-venues-filters" style="padding: 0 var(--space-lg) var(--space-lg); display: flex; gap: var(--space-sm); overflow-x: auto;">
                    ...
                    <button class="filter-chip" data-filter="volleyball">Volleyball</button>
                    <button class="filter-chip" data-filter="open-ground">Open Grounds</button>
                    <button class="filter-chip" data-filter="sports-facility">Sports Facilities</button>
                </div>
            ...
            document.querySelectorAll('.filter-chip').forEach(chip => {
                chip.addEventListener('click', function() {
                    ...
                    filterVenuesBySport(filterValue);
                });
            });
        }
        
        // Reset filters when loading page
        currentFilter = 'all';
        currentSearchTerm = '';
        document.querySelector('.filter-chip.active')?.classList.remove('active');
        document.querySelector('.filter-chip[data-filter="all"]')?.classList.add('active');
        const searchInput = document.getElementById('all-venues-search');
        if (searchInput) searchInput.value = '';
        
        await displayAllVenues('all', '');
        hideLoading();
        ...
        showToast('Failed to load venues and grounds', 'error');
    }
}

// ==================== DISPLAY ALL VENUES (CORRECTED) ====================
...
async function displayAllVenues(filterSport = 'all', searchTerm = '') {
    ...
    try {
        container.innerHTML = `
            <div class="loading-spinner">
                <div class="loader-spinner"></div>
                <p style="margin-top: var(--space-md); color: var(--gray-500);">Loading venues and grounds...</p>
            </div>
        `;
        
        // Get both venues and grounds in parallel for better performance
        const [venuesSnapshot, groundsSnapshot] = await Promise.all([
            db.collection(COLLECTIONS.VENUES)
                .where('hidden', '==', false)
                .get(),
            db.collection(COLLECTIONS.GROUNDS)
                .where('status', '==', 'active')
                .get()
        ]);
        ...
        // Process grounds - need to get owner info for each
        for (const doc of groundsSnapshot.docs) {
            const ground = doc.data();
            
            // Get owner info to determine if it's a plot owner
            let ownerType = 'venue_owner';
            try {
                const ownerDoc = await db.collection(COLLECTIONS.OWNERS).doc(ground.ownerId).get();
                if (ownerDoc.exists) {
                    ownerType = ownerDoc.data().ownerType || 'venue_owner';
                }
            } catch (e) {
                console.warn('Could not fetch owner for ground:', ground.groundName);
            }
            ...
            allItems.push({
                ...
                address: ground.groundAddress || 'Address not specified',
                city: ground.city || 'Location available',
                images: ground.images || [],
                rating: ground.rating || 0,
                totalReviews: ground.totalReviews || 0,
                location: ground.location || null,
        ...
        // Filter by sport or type
        if (filterSport !== 'all') {
            if (filterSport === 'open-ground') {
                filteredItems = filteredItems.filter(item => item.ownerType === 'plot_owner');
            } else if (filterSport === 'sports-facility') {
                filteredItems = filteredItems.filter(item => item.ownerType === 'venue_owner');
            } else {
                filteredItems = filteredItems.filter(item => 
                    item.sportType && item.sportType.toLowerCase() === filterSport.toLowerCase()
                );
            }
        }
        ...
        if (filteredItems.length === 0) {
            container.innerHTML = `
                <div class="empty-state">
                    ...
                    <button class="auth-btn" onclick="displayAllVenues('all', '')" style="margin-top: var(--space-lg);">Clear Filters</button>
                </div>
            `;
            return;
        }
        
        // Display items with professional styling
        container.innerHTML = filteredItems.map(item => {
            let distanceText = '';
            if (item.distance && item.distance !== Infinity) {
                distanceText = `<div class="venue-distance" style="font-size: var(--font-xs); color: var(--gray-500); margin-top: 4px;">
                    <i class="fas fa-location-dot"></i> ${item.distance.toFixed(1)} km away
                </div>`;
            }
            
            const verifiedBadge = item.isVerified ? 
                '<span class="verified-badge" style="margin-left: var(--space-sm); font-size: var(--font-xs); color: var(--success);"><i class="fas fa-check-circle"></i> Verified</span>' : '';
            
            const isPlotOwner = item.ownerType === 'plot_owner';
            const itemTypeIcon = isPlotOwner ? '<i class="fas fa-tree"></i>' : '<i class="fas fa-building"></i>';
            const itemTypeLabel = isPlotOwner ? 'Open Ground' : 'Sports Facility';
            const itemTypeColor = isPlotOwner ? 'var(--success)' : 'var(--primary)';
            const priceDisplay = (item.pricePerHour && !isNaN(item.pricePerHour) && item.pricePerHour > 0) ? 
                `<span class="price-badge" style="margin-left: 8px; background: var(--gray-100); color: var(--gray-700); padding: 2px 10px; border-radius: 20px; font-size: 11px;">
                    ${formatCurrency(item.pricePerHour)}/hr
                </span>` : '';
            
            return `
                <div class="venue-card" data-${item.type}-id="${item.id}" data-type="${item.type}" style="cursor: pointer;">
                    <img src="${item.images?.[0] || 'https://via.placeholder.com/120'}" 
                         alt="${escapeHtml(item.name)}" 
                         class="venue-image"
                         style="width: 120px; height: 120px; object-fit: cover;"
                         onerror="this.src='https://via.placeholder.com/120'">
                    <div class="venue-info" style="flex: 1; padding: var(--space-md);">
                        <div style="display: flex; align-items: center; flex-wrap: wrap; gap: var(--space-xs);">
                            <h3 style="font-size: var(--font-base); font-weight: 600; margin: 0;">${escapeHtml(item.name)}</h3>
                            ${verifiedBadge}
                        </div>
                        <div class="venue-sport" style="font-size: var(--font-xs); color: var(--primary); margin: var(--space-xs) 0;">
                            ${item.sportType || 'Multi-sport'}
                        </div>
                        <div class="venue-rating" style="margin: var(--space-xs) 0;">
                            <i class="fas fa-star" style="color: var(--warning);"></i> 
                            <span>${(item.rating || 0).toFixed(1)}</span>
                            <span style="color: var(--gray-500);">(${item.totalReviews || 0} reviews)</span>
                        </div>
                        <div class="venue-address" style="font-size: var(--font-xs); color: var(--gray-500); margin: var(--space-xs) 0;">
                            <i class="fas fa-map-marker-alt"></i> ${escapeHtml(item.address || 'Address not available')}
                        </div>
                        ${distanceText}
                        <div class="venue-type-badge" style="margin-top: 8px; display: flex; align-items: center; gap: 8px; flex-wrap: wrap;">
                            <span class="badge" style="background: ${itemTypeColor}20; color: ${itemTypeColor}; padding: 2px 10px; border-radius: 20px; font-size: 11px;">
                                ${itemTypeIcon} ${itemTypeLabel}
                            </span>
                            ${priceDisplay}
                        </div>
                    </div>
        ...
        document.querySelectorAll('.venue-card[data-venue-id]').forEach(card => {
            ...
            newCard.addEventListener('click', () => {
                viewVenue(newCard.dataset.venueId);
            });
        });
        ...
        document.querySelectorAll('.venue-card[data-ground-id]').forEach(card => {
            ...
            card.parentNode.replaceChild(newCard, card);
            newCard.addEventListener('click', () => {
                viewGround(newCard.dataset.groundId);
            });
        });
        ...
        container.innerHTML = `
            <div class="error-state" style="text-align: center; padding: var(--space-4xl);">
                <i class="fas fa-exclamation-circle" style="font-size: 3rem; color: var(--danger); margin-bottom: var(--space-lg);"></i>
                <h3>Unable to load venues and grounds</h3>
                <p>${error.message || 'Please try again later'}</p>
                <button class="retry-btn" onclick="displayAllVenues('${filterSport}', '${searchTerm}')" style="margin-top: var(--space-lg);">Retry</button>
            </div>
        `;
    }
}
...
// ==================== CHECK GROUND OWNER TYPE ====================

async function checkGroundOwnerType(groundId) {
    try {
        const groundDoc = await db.collection(COLLECTIONS.GROUNDS).doc(groundId).get();
        if (!groundDoc.exists) return { isPlotOwner: false, ownerData: null };
        
        const ground = groundDoc.data();
        const ownerDoc = await db.collection(COLLECTIONS.OWNERS).doc(ground.ownerId).get();
        
        if (!ownerDoc.exists) return { isPlotOwner: false, ownerData: null };
        
        const owner = ownerDoc.data();
        const isPlotOwner = owner.ownerType === OWNER_TYPES.PLOT_OWNER;
        ...
        console.error('Error checking ground owner type:', error);
        return { isPlotOwner: false, ownerData: null };
    }
}
// ==================== SEARCH ====================
...
// ==================== HANDLE ADD GROUND (BASE64 ONLY - NO FIREBASE STORAGE) ====================
...
async function handleAddGround(e) {
    ...
    // Show upload progress (optional, for UX)
    const uploadProgress = document.getElementById('upload-progress');
    const progressFill = document.getElementById('upload-progress-fill');
    const uploadStatus = document.getElementById('upload-status');
    ...
    if (uploadProgress) uploadProgress.style.display = 'block';
    if (progressFill) progressFill.style.width = '0%';
    if (uploadStatus) uploadStatus.textContent = 'Converting images to Base64...';
    ...
    try {
        ...
        // Prepare ground data with Base64 images
        const groundData = {
            ownerId: currentUser.uid,
            groundName: groundName,
            sportType: sportType,
            pricePerHour: pricePerHour,
            groundAddress: groundAddress || '',
            images: imageBase64, // Store Base64 strings directly in Firestore
            rating: 0,
            totalReviews: 0,
            status: 'active',
            isVerified: false,
        ...
        // Add ground to Firestore
        const groundRef = await db.collection(COLLECTIONS.GROUNDS).add(groundData);
        console.log('Ground added with ID:', groundRef.id);
        ...
        const message = groundImages.length === 0 ? 
            'Ground added successfully! You can add photos later from the ground management page.' : 
            `Ground added successfully with ${groundImages.length} photo(s)!`;
        showToast(message, 'success');
...
// Add this after the RAZORPAY_KEY_ID constant (around line 30-40)

// ==================== PAYTM PAYMENT INTEGRATION ====================
// Paytm Business Configuration
const PAYTM_MID = 'YOUR_PAYTM_MERCHANT_ID'; // Replace with your actual Paytm Merchant ID
const PAYTM_MERCHANT_KEY = 'YOUR_MERCHANT_KEY'; // Replace with your actual Merchant Key
const PAYTM_WEBSITE = 'WEBSTAGING'; // Use 'WEBSTAGING' for testing, 'DEFAULT' for production

// Determine API endpoint based on environment
const PAYTM_ENV = 'TEST'; // Change to 'PROD' for production
const PAYTM_API_URL = PAYTM_ENV === 'TEST' 
    ? 'https://securegw-stage.paytm.in/theia/api/v1/initiateTransaction' 
    : 'https://securegw.paytm.in/theia/api/v1/initiateTransaction';

const PAYTM_STATUS_URL = PAYTM_ENV === 'TEST'
    ? 'https://securegw-stage.paytm.in/merchant-status/getTxnStatus'
    : 'https://securegw.paytm.in/merchant-status/getTxnStatus';

// Update the showTournamentPayment function to use Paytm instead of Razorpay
// Replace your showTournamentPayment function with this corrected version

function showTournamentPayment(tournament, registrationId, teamName) {
    console.log('showTournamentPayment called with:', { tournament, registrationId, teamName });
    
    // Ensure tournament has proper id
    let tournamentData = tournament;
    
    // If tournament is a Firestore document or doesn't have id
    if (!tournamentData.id && tournamentData.tournamentId) {
        tournamentData.id = tournamentData.tournamentId;
    }
    
    // If tournament is provided as an object with id property
    if (tournamentData && !tournamentData.id && tournamentData._document) {
        tournamentData.id = tournamentData._document.key.path.segments[tournamentData._document.key.path.segments.length - 1];
    }
    
    // If still no id, try to get from currentTournament
    if (!tournamentData.id && currentTournament && currentTournament.id) {
        tournamentData = currentTournament;
    }
    
    // If still no id, show error
    if (!tournamentData || !tournamentData.id) {
        console.error('Tournament ID not found:', tournamentData);
        showToast('Error: Tournament information missing. Please try again.', 'error');
        return;
    }
    
    const amount = tournamentData.entryFee;
        ...
        <div class="tournament-payment-container">
            <div class="payment-card">
                ...
                <div class="tournament-summary-card">
                    ...
                    <div class="summary-details">
                        <div class="summary-detail-item">
                            ...
                            <div class="summary-detail-content">
                                <span class="summary-detail-label">Tournament Name</span>
                                <span class="summary-detail-value">${escapeHtml(tournamentData.tournamentName)}</span>
                            </div>
                        </div>
                        <div class="summary-detail-item">
                            <i class="fas fa-calendar-alt"></i>
                ...
                <div class="upi-payment-section">
                    ...
                    <div class="upi-apps-grid-modern">
                        <div class="upi-app-modern" data-payment-type="paytm">
                            <img src="https://www.google.com/imgres?q=paytm%20wallet%20logo&imgurl=https%3A%2F%2Flogos-world.net%2Fwp-content%2Fuploads%2F2020%2F11%2FPaytm-Symbol.png&imgrefurl=https%3A%2F%2Flogos-world.net%2Fpaytm-logo%2F&docid=Tpx9t0jJcEcKpM&tbnid=cVj9gvWjdXQNDM&vet=12ahUKEwiE1ojKhraTAxWCWXADHZQmFdoQnPAOegQIIBAB..i&w=3840&h=2160&hcb=2&ved=2ahUKEwiE1ojKhraTAxWCWXADHZQmFdoQnPAOegQIIBAB" alt="Paytm" class="upi-logo-modern">
                            <span>Paytm Wallet</span>
                        </div>
                        <div class="upi-app-modern" data-payment-type="upi">
                            <img src="https://upload.wikimedia.org/wikipedia/commons/thumb/e/e1/Amazon_Pay_Logo.svg/1200px-Amazon_Pay_Logo.svg.png" alt="UPI" class="upi-logo-modern">
                            <span>UPI</span>
                        </div>
                        <div class="upi-app-modern" data-payment-type="card">
                            <i class="fas fa-credit-card" style="font-size: 2rem; color: var(--primary);"></i>
                            <span>Credit/Debit Card</span>
                        </div>
                        <div class="upi-app-modern" data-payment-type="netbanking">
...
// Replace your initiatePaytmPayment function with this corrected version

// ==================== FIXED INITIATE PAYTM PAYMENT ====================
// ==================== FIXED INITIATE PAYTM PAYMENT ====================
async function initiatePaytmPayment(tournament, registrationId, teamName, paymentType = 'paytm') {
    console.log('=== PAYTM PAYMENT DEBUG START ===');
    console.log('initiatePaytmPayment called with:', { 
        tournament, 
        registrationId, 
        teamName, 
        paymentType,
        tournamentType: typeof tournament
    });
    ...
    try {
        // Get tournament details - handle different possible formats
        ...
        function extractId(obj) {
            ...
            if (obj.tournamentId) return obj.tournamentId;
            return null;
        }
        
        // CASE 1: If tournament is provided as an object with id or tournamentId
        if (tournament && typeof tournament === 'object') {
            tournamentData = tournament;
            tournamentId = extractId(tournamentData);
            console.log('CASE 1: Tournament from parameter, ID:', tournamentId);
        }
        
        // CASE 2: If we have tournamentId from registrationId
        if (!tournamentId && registrationId) {
            const pendingRegStr = sessionStorage.getItem('pendingTournamentRegistration');
            if (pendingRegStr) {
                try {
                    ...
                    if (pendingReg.tournamentId) {
                        tournamentId = pendingReg.tournamentId;
                        console.log('CASE 2: Got tournamentId from pending registration:', tournamentId);
                        // Store for later use
                        if (!tournamentData) {
                            tournamentData = { id: tournamentId, tournamentName: pendingReg.tournamentName };
                        }
        ...
        // CASE 3: If we have tournamentId from window.currentTournamentPayment
        if (!tournamentId && window.currentTournamentPayment && window.currentTournamentPayment.tournamentId) {
            tournamentId = window.currentTournamentPayment.tournamentId;
            console.log('CASE 3: Got tournamentId from window.currentTournamentPayment:', tournamentId);
            if (!tournamentData && window.currentTournamentPayment.tournament) {
                tournamentData = window.currentTournamentPayment.tournament;
            }
        }
        
        // CASE 4: If we have tournamentId from currentTournament
        if (!tournamentId && currentTournament) {
            tournamentId = extractId(currentTournament);
            console.log('CASE 4: Got tournamentId from currentTournament:', tournamentId);
            if (!tournamentData) {
                tournamentData = currentTournament;
            }
        }
        
        // CASE 5: If tournament is a string (ID), treat it as ID
        if (!tournamentId && tournament && typeof tournament === 'string') {
            tournamentId = tournament;
            console.log('CASE 5: Tournament ID from string parameter:', tournamentId);
        }
        
        // Now we have tournamentId, fetch the full tournament data if needed
        if (tournamentId && (!tournamentData || !tournamentData.tournamentName)) {
            console.log('Fetching full tournament data for ID:', tournamentId);
            const tournamentDoc = await db.collection(COLLECTIONS.TOURNAMENTS).doc(tournamentId).get();
            if (tournamentDoc.exists) {
                tournamentData = { id: tournamentDoc.id, ...tournamentDoc.data() };
                console.log('Tournament fetched successfully:', tournamentData.tournamentName);
            } else {
                // Try to search by tournamentId field
                const altSnapshot = await db.collection(COLLECTIONS.TOURNAMENTS)
                    .where('tournamentId', '==', tournamentId)
                    .limit(1)
                    .get();
                if (!altSnapshot.empty) {
                    tournamentData = { id: altSnapshot.docs[0].id, ...altSnapshot.docs[0].data() };
                    tournamentId = tournamentData.id;
                    console.log('Found tournament by tournamentId field:', tournamentData.tournamentName);
                }
            }
        }
        ...
        if (!tournamentData) {
            console.error('No tournament data available. Debug info:', {
                tournamentParam: tournament,
                currentTournament: currentTournament,
                windowCurrentTournamentPayment: window.currentTournamentPayment,
                registrationId: registrationId,
                sessionStorageKeys: Object.keys(sessionStorage),
                pendingTournament: sessionStorage.getItem('pendingTournamentRegistration')
            });
            throw new Error('Tournament information not found. Please go back and try again.');
        }
        
        // Get tournament ID - ensure we have it
        if (!tournamentId) {
            tournamentId = tournamentData.id || tournamentData.tournamentId;
        }
        
        // If still no tournamentId, show error
        if (!tournamentId) {
            console.error('Tournament data missing ID:', tournamentData);
            throw new Error('Tournament ID not found. Please refresh and try again.');
        }
        
        console.log('Final Tournament ID:', tournamentId);
        console.log('Final Tournament name:', tournamentData.tournamentName);
        ...
        if (!freshTournamentDoc.exists) {
            console.error('Tournament not found in Firestore with ID:', tournamentId);
            // Try to fetch all tournaments to help debug
            const allTournaments = await db.collection(COLLECTIONS.TOURNAMENTS).limit(5).get();
            console.log('Sample tournament IDs in database:');
            allTournaments.forEach(doc => {
                console.log('-', doc.id, doc.data().tournamentName);
            });
            throw new Error(`Tournament not found. Please go back and try again.`);
        }
        ...
        const amount = freshTournament.entryFee;
        ...
        const orderData = {
            ...
            registrationId: registrationId,
            tournamentId: tournamentId,
            tournamentName: freshTournament.tournamentName,
            teamName: teamName,
            amount: amount,
            userId: currentUser.uid,
            userName: currentUser.name || currentUser.ownerName || 'User',
        ...
        await db.collection('paytm_orders').add(orderData);
        
        // Generate unique transaction ID
        const transactionId = generateMatchTransactionId('PAYTM');
        ...
        // Store payment request in session storage for callback
        sessionStorage.setItem('pendingTournamentPayment', JSON.stringify({
            registrationId: registrationId,
            tournamentId: tournamentId,
            tournamentName: freshTournament.tournamentName,
            teamName: teamName,
        ...
        showPaytmPaymentInstructions(registrationId, tournamentId, teamName, amount, orderId, paymentType);
...
function generatePaytmOrderId() {
    ...
    const random = Math.floor(Math.random() * 1000000);
    const randomStr = random.toString().padStart(6, '0');
    return `BOOKMYGAME_${timestamp}_${randomStr}`;
}

// ==================== REDIRECT TO PAYTM PAYMENT ====================

async function redirectToPaytmPayment(paymentRequest, registrationId, tournamentId, teamName, amount) {
    showLoading('Redirecting to Paytm...');
    
    try {
        // In a production environment, you would make a server-side call to get the payment URL
        // For demonstration, we'll simulate the redirect with a modal showing payment instructions
        
        // For actual integration, you would call a cloud function that initiates the payment with Paytm
        // const initiatePaytmPayment = firebase.functions().httpsCallable('initiatePaytmPayment');
        // const result = await initiatePaytmPayment(paymentRequest);
        // window.location.href = result.data.paytmUrl;
        
        // For now, show a payment instructions modal with Paytm QR code
        showPaytmPaymentInstructions(registrationId, tournamentId, teamName, amount, paymentRequest.orderId);
        
    } catch (error) {
        hideLoading();
        console.error('Redirect error:', error);
        showToast('Unable to redirect to payment page. Please try again.', 'error');
    }
}


function showPaytmPaymentInstructions(registrationId, tournamentId, teamName, amount, orderId, paymentType) {
    console.log('showPaytmPaymentInstructions called with:', { registrationId, tournamentId, teamName, amount, orderId, paymentType });
    
    // Get tournament details
    db.collection(COLLECTIONS.TOURNAMENTS).doc(tournamentId).get().then(tournamentDoc => {
        if (!tournamentDoc.exists) return;
        const tournament = tournamentDoc.data();
        
        let modal = document.getElementById('paytm-payment-instructions-modal');
        
        if (!modal) {
            const modalHtml = `
                <div id="paytm-payment-instructions-modal" class="modal">
                    <div class="modal-content" style="max-width: 450px;">
                        <div class="modal-header">
                            <h3><i class="fas fa-credit-card"></i> Complete Payment</h3>
                            <button class="close-btn" id="close-paytm-payment-modal">&times;</button>
                        </div>
                        <div class="modal-body">
                            <div class="payment-instructions-container">
                                <div class="payment-amount-highlight" style="background: linear-gradient(135deg, #1b2e6c, #0b1a3e); color: white; text-align: center; padding: var(--space-xl); border-radius: var(--radius); margin-bottom: var(--space-xl);">
                                    <div class="payment-label" style="font-size: var(--font-sm); opacity: 0.9;">Amount to Pay</div>
                                    <div class="payment-amount" style="font-size: 2rem; font-weight: 800;">${formatCurrency(amount)}</div>
                                    <div class="payment-note" style="font-size: var(--font-xs); opacity: 0.8;">Tournament Registration Fee</div>
                                </div>
                                ...
                                <div class="payment-instructions-card" style="background: var(--white); border: 1px solid var(--gray-200); border-radius: var(--radius); padding: var(--space-xl); margin-bottom: var(--space-xl);">
                                    <h4 style="margin-bottom: var(--space-lg); display: flex; align-items: center; gap: var(--space-sm);">
                                        <i class="fas fa-mobile-alt" style="color: #1b2e6c;"></i>
                                        Pay using Paytm
                                    </h4>
                                    
                                    <div class="instruction-step" style="display: flex; align-items: center; gap: var(--space-md); margin-bottom: var(--space-lg);">
                                        <div class="step-number" style="width: 32px; height: 32px; background: #1b2e6c; color: white; border-radius: 50%; display: flex; align-items: center; justify-content: center; font-weight: 600;">1</div>
                                        <div class="step-text">Open Paytm app on your phone</div>
                                    </div>
                                    
                                    <div class="instruction-step" style="display: flex; align-items: center; gap: var(--space-md); margin-bottom: var(--space-lg);">
                                        <div class="step-number" style="width: 32px; height: 32px; background: #1b2e6c; color: white; border-radius: 50%; display: flex; align-items: center; justify-content: center; font-weight: 600;">2</div>
                                        <div class="step-text">Click on the payment link sent to your phone</div>
                                    </div>
                                    
                                    <div class="instruction-step" style="display: flex; align-items: center; gap: var(--space-md); margin-bottom: var(--space-lg);">
                                        <div class="step-number" style="width: 32px; height: 32px; background: #1b2e6c; color: white; border-radius: 50%; display: flex; align-items: center; justify-content: center; font-weight: 600;">3</div>
                                        <div class="step-text">Complete the payment using your preferred method</div>
                                    </div>
                                    
                                    <div class="instruction-step" style="display: flex; align-items: center; gap: var(--space-md);">
                                        <div class="step-number" style="width: 32px; height: 32px; background: #1b2e6c; color: white; border-radius: 50%; display: flex; align-items: center; justify-content: center; font-weight: 600;">4</div>
                                        <div class="step-text">Wait for automatic confirmation</div>
                                    </div>
                                </div>
                                
                                <div class="payment-note-box" style="background: var(--primary-50); padding: var(--space-md); border-radius: var(--radius); margin-bottom: var(--space-xl); display: flex; gap: var(--space-sm);">
                                    <i class="fas fa-clock" style="color: var(--primary);"></i>
                                    <p style="margin: 0; font-size: var(--font-sm);">Your payment will be automatically verified. Please wait for confirmation.</p>
                                </div>
                                
                                <div class="payment-actions" style="display: flex; gap: var(--space-md);">
                                    <button class="payment-verify-btn" id="check-payment-status-btn" style="flex: 2; background: linear-gradient(135deg, #1b2e6c, #0b1a3e); color: white; border: none; border-radius: var(--radius); padding: var(--space-md); font-weight: 600; cursor: pointer;">
                                        <i class="fas fa-sync-alt"></i> Check Payment Status
                                    </button>
                                    <button class="payment-cancel-btn" id="cancel-paytm-payment-btn" style="flex: 1; background: var(--gray-100); color: var(--gray-700); border: none; border-radius: var(--radius); padding: var(--space-md); font-weight: 600; cursor: pointer;">
                                        <i class="fas fa-times"></i> Cancel
                                    </button>
                                </div>
                            </div>
            ...
            document.getElementById('close-paytm-payment-modal').addEventListener('click', async () => {
                await cancelPendingRegistration(registrationId);
                closeModal('paytm-payment-instructions-modal');
                sessionStorage.removeItem('pendingTournamentPayment');
                goBack();
            });
            
            document.getElementById('cancel-paytm-payment-btn').addEventListener('click', async () => {
                await cancelPendingRegistration(registrationId);
                closeModal('paytm-payment-instructions-modal');
                sessionStorage.removeItem('pendingTournamentPayment');
                goBack();
            ...
            document.getElementById('check-payment-status-btn').addEventListener('click', async () => {
                await checkPaymentStatus(orderId, registrationId, amount);
            });
        }
        
        const modalEl = document.getElementById('paytm-payment-instructions-modal');
        modalEl.classList.add('active');
        
        // Start polling for payment status
        startPaymentPolling(orderId, registrationId, amount);
        
    }).catch(error => {
        console.error('Error getting tournament details:', error);
        showToast('Error loading payment page. Please try again.', 'error');
    });
}
...
function startPaymentPolling(orderId, registrationId, amount) {
    // Clear any existing interval
    if (paymentPollingInterval) {
        clearInterval(paymentPollingInterval);
    }
    
    // Poll every 5 seconds for 2 minutes (24 attempts)
    let attempts = 0;
    ...
    paymentPollingInterval = setInterval(async () => {
        ...
        const modal = document.getElementById('paytm-payment-instructions-modal');
        if (!modal || !modal.classList.contains('active')) {
            clearInterval(paymentPollingInterval);
        ...
        if (attempts >= maxAttempts) {
            clearInterval(paymentPollingInterval);
            paymentPollingInterval = null;
            showToast('Payment taking longer than expected. Please check status manually.', 'info');
            return;
        }
...
async function checkPaymentStatus(orderId, registrationId, amount) {
    try {
        // Call the secure cloud function to verify payment
        const verifyPaytmPayment = firebase.functions().httpsCallable('verifyPaytmPayment');
        
        const result = await verifyPaytmPayment({
            orderId: orderId,
            registrationId: registrationId
        });
        ...
        if (result.data.success) {
            ...
            // Show success modal
            showPaytmSuccessModal(registrationId, amount, result.data.tournamentName, result.data.teamName, orderId);
            
            // Clear session storage
            sessionStorage.removeItem('pendingTournamentRegistration');
            sessionStorage.removeItem('pendingTournamentPayment');
            
            // Refresh tournament listings if needed
            if (document.getElementById('tournaments-page') && document.getElementById('tournaments-page').classList.contains('active')) {
                loadAllTournaments('upcoming');
            }
        ...
        // Don't clear polling on error, just log
    }
}

// Firebase Cloud Function for Paytm Payment Verification
//const functions = require('firebase-functions');
const admin = require('firebase-admin');
const crypto = require('crypto');
const axios = require('axios');

exports.verifyPaytmPayment = functions.https.onCall(async (data, context) => {
    ...
    try {
        ...
        const orderDoc = await admin.firestore().collection('paytm_orders').doc(orderId).get();
        ...
        // Paytm Configuration
        const PAYTM_MID = functions.config().paytm.mid;
        const PAYTM_MERCHANT_KEY = functions.config().paytm.merchant_key;
        const PAYTM_ENV = functions.config().paytm.env || 'TEST';
        ...
        const checksum = generatePaytmChecksum(checksumParams, PAYTM_MERCHANT_KEY);
        
        // Call Paytm API to verify payment
        const response = await axios.post(PAYTM_STATUS_URL, {
            MID: PAYTM_MID,
            ORDERID: orderId,
        ...
        if (paymentStatus.STATUS === 'TXN_SUCCESS') {
            // Payment successful - create registration
            const pendingRegQuery = await admin.firestore()
                .collection('pending_tournament_registrations')
                .where('registrationId', '==', registrationId)
                .where('userId', '==', userId)
                .get();
            
            if (pendingRegQuery.empty) {
                throw new Error('Pending registration not found');
            }
            
            const pendingReg = pendingRegQuery.docs[0].data();
            const pendingRegRef = pendingRegQuery.docs[0].ref;
            
            // Check if tournament is still available
            const tournamentRef = admin.firestore().collection('tournaments').doc(pendingReg.tournamentId);
            const tournamentDoc = await tournamentRef.get();
            ...
            // Check if tournament is full
            if (tournament.registeredTeams && tournament.registeredTeams.length >= tournament.maxTeams) {
            ...
            const registrationData = {
                ...
                userEmail: pendingReg.userEmail,
                userPhone: pendingReg.userPhone,
                teamName: pendingReg.teamName,
                captainName: pendingReg.captainName,
                captainPhone: pendingReg.captainPhone,
                contactNumber: pendingReg.contactNumber,
                players: pendingReg.players,
                entryFee: pendingReg.entryFee,
                status: 'confirmed',
                paymentStatus: 'success',
                orderId: orderId,
                paytmTxnId: paymentStatus.TXNID,
                paytmBankTxnId: paymentStatus.BANKTXNID,
            ...
            await admin.firestore().collection('tournament_registrations').add(registrationData);
            
            // Update tournament with registered team
            await tournamentRef.update({
                registeredTeams: admin.firestore.FieldValue.arrayUnion({
                    teamName: pendingReg.teamName,
                    userId: pendingReg.userId,
                    userName: pendingReg.userName,
                    captainName: pendingReg.captainName,
            ...
            await orderDoc.ref.update({
                status: 'failed',
                failureReason: paymentStatus.RESPMSG,
                updatedAt: admin.firestore.FieldValue.serverTimestamp()
            });
...
function generatePaytmChecksum(params, merchantKey) {
    ...
    sortedKeys.forEach(key => {
        checksumString += key + '=' + params[key] + '&';
    });
    
    checksumString = checksumString.slice(0, -1);
    checksumString += '&' + merchantKey;
...
async function cancelPendingRegistration(registrationId) {
    try {
        // Delete from Firestore pending collection
        const pendingRegQuery = await db.collection(COLLECTIONS.PENDING_TOURNAMENT_REGISTRATIONS)
            .where('registrationId', '==', registrationId)
            .where('userId', '==', currentUser.uid)
            .get();
        
        if (!pendingRegQuery.empty) {
            await pendingRegQuery.docs[0].ref.delete();
            console.log('Pending registration cancelled:', registrationId);
        }
...
async function cleanupExpiredPendingRegistrations() {
    try {
        ...
        const expiredRegistrations = await db.collection(COLLECTIONS.PENDING_TOURNAMENT_REGISTRATIONS)
            .where('createdAt', '<=', thirtyMinutesAgo)
            .where('status', '==', 'pending')
            .get();
        
        const batch = db.batch();
        expiredRegistrations.forEach(doc => {
            batch.delete(doc.ref);
        });
        
        if (expiredRegistrations.size > 0) {
            await batch.commit();
            console.log(`Cleaned up ${expiredRegistrations.size} expired pending registrations`);
        }
...
// Add this check at the beginning of showTournamentRegistration function

// ==================== OPTIMIZED SHOW TOURNAMENT REGISTRATION ====================
...
async function showTournamentRegistration(tournamentId) {
    ...
    if (container) {
        container.innerHTML = `
            <div class="loading-spinner" style="padding: var(--space-4xl);">
                <div class="loader-spinner"></div>
                <p style="margin-top: var(--space-md); color: var(--gray-500);">Loading registration form...</p>
            </div>
        `;
    }
    ...
    try {
        // Fetch tournament details - single query
        const tournamentDoc = await db.collection(COLLECTIONS.TOURNAMENTS).doc(tournamentId).get();
        ...
        const tournament = tournamentDoc.data();
        currentTournament = { id: tournamentDoc.id, ...tournament };
        
        // Run checks in parallel for better performance
        const [pendingRegCheck, existingRegCheck] = await Promise.all([
            // Check for pending registration
            db.collection(COLLECTIONS.PENDING_TOURNAMENT_REGISTRATIONS)
                .where('tournamentId', '==', tournamentId)
                .where('userId', '==', currentUser.uid)
                .where('status', '==', 'pending')
                .limit(1)
        ...
        if (!pendingRegCheck.empty) {
            const pendingReg = pendingRegCheck.docs[0].data();
            const createdAt = pendingReg.createdAt?.toDate() || new Date();
            const minutesAgo = Math.floor((Date.now() - createdAt) / 60000);
            
            const confirmResume = confirm(
                `You have a pending registration from ${minutesAgo} minutes ago.\n\n` +
                `Would you like to continue with that payment?`
            );
            
            if (confirmResume) {
                hideLoading();
                // In showTournamentRegistration, when calling showTournamentPayment:
showTournamentPayment(tournamentData, pendingRegistration.registrationId, pendingRegistration.teamName);
                return;
            } else {
                // Delete the pending registration
                await pendingRegCheck.docs[0].ref.delete();
                sessionStorage.removeItem('pendingTournamentRegistration');
            }
        }
        
        // Handle existing confirmed registration
        if (!existingRegCheck.empty) {
            hideLoading();
            showAlreadyRegisteredModal(existingRegCheck.docs[0].data());
            return;
        }
        
        // Get user info - use currentUser directly for speed
        const userName = currentUser.name || currentUser.ownerName || currentUser.displayName || 
        ...
        // Generate player list HTML efficiently
        let playersHtml = '';
        for (let i = 1; i < teamSize; i++) {
            playersHtml += `
                <div class="player-item-modern" data-player-index="${i}">
        ...
        // Build HTML with minimal DOM manipulation
        const html = `
            <div class="tournament-registration-container">
                <div class="registration-card-modern">
                    <!-- Tournament Header Banner -->
        ...
        // Add player input validation with event delegation for performance
        const playerContainer = document.getElementById('player-list-container');
        if (playerContainer) {
            playerContainer.addEventListener('input', (e) => {
                if (e.target.classList && e.target.classList.contains('player-input-modern')) {
        ...
        hideLoading();
        console.error('Error loading tournament registration:', error);
        showToast('Error loading registration form: ' + error.message, 'error');
        ...
        if (container) {
            container.innerHTML = `
                <div class="error-state" style="text-align: center; padding: var(--space-4xl);">
                    ...
                    <p>${error.message || 'Please try again later'}</p>
                    <button class="auth-btn" onclick="goBack()" style="margin-top: var(--space-lg);">Go Back</button>
                </div>
            `;
        }
    }
...
// ==================== SHOW PAYTM SUCCESS MODAL ====================

function showPaytmSuccessModal(registrationId, amount, tournamentName, teamName, orderId) {
    // Check if modal already exists
    let modal = document.getElementById('paytm-success-modal');
    ...
    const modalHtml = `
        <div id="paytm-success-modal" class="modal">
            <div class="modal-content" style="max-width: 400px;">
                <div class="modal-header">
                    <h3><i class="fas fa-check-circle" style="color: var(--success);"></i> Registration Confirmed!</h3>
                    <button class="close-btn" id="close-paytm-success-modal">&times;</button>
                </div>
                <div class="modal-body">
                    <div style="text-align: center;">
                        <div style="width: 80px; height: 80px; background: linear-gradient(135deg, #1b2e6c, #0b1a3e); border-radius: 50%; display: flex; align-items: center; justify-content: center; margin: 0 auto var(--space-xl);">
                            <i class="fas fa-check-circle" style="font-size: 2.5rem; color: white;"></i>
                        </div>
                        ...
                        <div style="display: flex; gap: var(--space-md);">
                            <button id="view-tournament-paytm-btn" style="flex: 1; background: linear-gradient(135deg, #1b2e6c, #0b1a3e); color: white; border: none; border-radius: var(--radius); padding: var(--space-md); font-weight: 600; cursor: pointer; transition: all 0.2s ease;">
                                <i class="fas fa-eye"></i> View Tournament
                            </button>
                            <button id="paytm-go-home" style="flex: 1; background: #f3f4f6; color: #374151; border: none; border-radius: var(--radius); padding: var(--space-md); font-weight: 600; cursor: pointer; transition: all 0.2s ease;">
                                <i class="fas fa-home"></i> Go Home
                            </button>
                        </div>
                    </div>
    ...
    modal = document.getElementById('paytm-success-modal');
    
    // Add close button event listener
    const closeBtn = document.getElementById('close-paytm-success-modal');
    if (closeBtn) {
        closeBtn.addEventListener('click', () => {
            closeModal('paytm-success-modal');
            goHome();
        });
    }
    
    // Add view tournament button event listener
    const viewTournamentBtn = document.getElementById('view-tournament-paytm-btn');
    ...
    const goHomeBtn = document.getElementById('paytm-go-home');
    if (goHomeBtn) {
...
// Add this at the beginning of initiateRazorpayPayment
console.log('=== INITIATE RAZORPAY PAYMENT DEBUG ===');
console.log('Tournament object received:', tournament);
console.log('Tournament keys:', tournament ? Object.keys(tournament) : 'null');
console.log('Tournament ID:', tournament?.id);
console.log('Tournament tournamentId:', tournament?.tournamentId);
...
async function initiateRazorpayPayment(tournament, registrationId, teamName) {
    ...
    try {
        ...
        // Get tournament ID - handle both id and tournamentId
        const tournamentId = tournamentData.id || tournamentData.tournamentId;
        
        if (!tournamentId) {
            console.error('Tournament data:', tournamentData);
        ...
        const options = {
            ...
            currency: 'INR',
            name: 'BookMyGame',
            description: `Tournament Registration: ${freshTournament.tournamentName}`,
            image: 'https://bookmygame.com/logo.png',
            order_id: orderId,
            handler: async function(response) {
                // Payment successful - create registration
                await handleRazorpayPaymentSuccess(response, registrationId, freshTournament, teamName, amount, transactionId);
            },
            prefill: {
...
function showPaymentFailedModal() {
            ...
            <div id="payment-failed-modal" class="modal">
                <div class="modal-content" style="max-width: 350px;">
                    ...
                    <div class="modal-body" style="text-align: center;">
                        ...
                        <p style="margin: var(--space-md) 0;">Your payment was not successful. Please try again or contact support.</p>
                        <div class="success-actions" style="display: flex; gap: var(--space-md);">
                            <button class="btn-primary-modern" id="retry-payment" style="background: linear-gradient(135deg, #1b2e6c, #0b1a3e);">
                                <i class="fas fa-redo"></i> Try Again
                            </button>
                            <button class="btn-secondary-modern" id="contact-support">
                                <i class="fas fa-headset"></i> Contact Support
                            </button>
                        </div>
                    </div>
    ...
    document.getElementById('retry-payment').onclick = () => {
        ...
        if (currentTournament) {
            showTournamentRegistration(currentTournament.id);
        } else {
            goBack();
        }
    };
...
async function loadReferralData() {
    ...
    try {
        ...
        const userRef = currentUser.role === 'user' ? COLLECTIONS.USERS : COLLECTIONS.OWNERS;
        const userDoc = await db.collection(userRef).doc(currentUser.uid).get();
        ...
        // Calculate earnings (₹50 per completed referral)
        const earnings = completedCount * 50;
        
        // Update UI elements
        const referralCountEl = document.getElementById('referral-count');
        const referralEarningsEl = document.getElementById('referral-earnings');
        const pendingReferralsEl = document.getElementById('pending-referrals');
        const inviteReferralCodeEl = document.getElementById('invite-referral-code');
        const historyCountEl = document.getElementById('history-count');
        
        if (referralCountEl) referralCountEl.textContent = completedCount;
        if (referralEarningsEl) referralEarningsEl.textContent = formatCurrency(earnings);
        if (pendingReferralsEl) pendingReferralsEl.textContent = pendingCount;
        if (inviteReferralCodeEl) inviteReferralCodeEl.textContent = referralCode;
        if (historyCountEl) historyCountEl.textContent = referrals.length;
            ...
            if (referrals.length === 0) {
                historyList.innerHTML = `
                    <div class="empty-history">
                        ...
                        <span>Share your code to get started</span>
                    </div>
                `;
            } else {
                historyList.innerHTML = referrals.map(ref => {
                    const createdAt = ref.createdAt?.toDate() || new Date(ref.createdAt);
                    ...
                    return `
                        <div class="history-item">
                            ...
                            <div class="history-info">
                                <span class="history-name">Friend Joined</span>
                                <span class="history-date">${formattedDate}</span>
                            </div>
                            <span class="history-status ${ref.status === 'completed' ? 'completed' : 'pending'}">
        ...
        // Generate share link
        const shareLink = `${window.location.origin}/index.html?ref=${referralCode}`;
        const shareLinkInput = document.getElementById('share-link-input');
        if (shareLinkInput) shareLinkInput.value = shareLink;
        
        // Generate QR Code
        generateReferralQRCode(shareLink);
...
async function generateReferralQRCode(link) {
    ...
    try {
        ...
        console.error('QR generation error:', error);
        qrContainer.innerHTML = `
            <div class="qr-error" style="text-align: center; color: var(--danger);">
                <i class="fas fa-exclamation-circle"></i>
...
function copyReferralCode() {
    const code = document.getElementById('invite-referral-code').textContent;
    navigator.clipboard.writeText(code);
    showToast('Referral code copied!', 'success');
}
...
function shareViaWhatsApp() {
    ...
    const message = `🎮 Join me on BookMyGame! Use my referral code ${code} to get started. Download the app at ${window.location.origin}`;
...
// Share via Instagram (opens share sheet)
function shareViaInstagram() {
    const message = `🎮 Join me on BookMyGame! Use my referral code ${window.currentReferralCode} to get started!`;
...
function shareViaMessenger() {
    const code = window.currentReferralCode;
    const message = `🎮 Join me on BookMyGame! Use my referral code ${code} to get started. Download the app at ${window.location.origin}`;
    window.open(`https://www.facebook.com/dialog/send?link=${encodeURIComponent(window.location.origin)}&app_id=YOUR_APP_ID&redirect_uri=${encodeURIComponent(window.location.origin)}`, '_blank');
}
...
function shareViaEmail() {
    const code = window.currentReferralCode;
    const subject = "Join me on BookMyGame!";
    const body = `Hi! I'm inviting you to join BookMyGame - the best sports booking platform!\n\nUse my referral code: ${code}\n\nDownload the app: ${window.location.origin}\n\nSee you on the field! 🏆`;
    window.location.href = `mailto:?subject=${encodeURIComponent(subject)}&body=${encodeURIComponent(body)}`;
}

// Share all (native share)
function shareAll() {
    const code = window.currentReferralCode;
    const message = `🎮 Join me on BookMyGame! Use my referral code ${code} to get started. Download the app at ${window.location.origin}`;
...
function initializeInviteFriends() {
    const inviteSection = document.getElementById('invite-friends-section');
...
function addInviteEventListeners() {
    // Copy code button
    const copyCodeBtn = document.getElementById('copy-invite-code');
    if (copyCodeBtn) {
        const newBtn = copyCodeBtn.cloneNode(true);
        copyCodeBtn.parentNode.replaceChild(newBtn, copyCodeBtn);
        newBtn.addEventListener('click', copyReferralCode);
    }
    ...
    if (copyLinkBtn) {
        ...
        newBtn.addEventListener('click', copyShareLink);
    }
    
    // Share all button
    const shareAllBtn = document.getElementById('share-all-btn');
    if (shareAllBtn) {
        const newBtn = shareAllBtn.cloneNode(true);
        shareAllBtn.parentNode.replaceChild(newBtn, shareAllBtn);
        newBtn.addEventListener('click', shareAll);
    }
    
    // WhatsApp share
    const whatsappBtn = document.getElementById('share-whatsapp-invite');
    if (whatsappBtn) {
        const newBtn = whatsappBtn.cloneNode(true);
        whatsappBtn.parentNode.replaceChild(newBtn, whatsappBtn);
        newBtn.addEventListener('click', shareViaWhatsApp);
    }
    
    // Telegram share
    const telegramBtn = document.getElementById('share-telegram-invite');
    if (telegramBtn) {
        const newBtn = telegramBtn.cloneNode(true);
        telegramBtn.parentNode.replaceChild(newBtn, telegramBtn);
        newBtn.addEventListener('click', shareViaTelegram);
    }
    
    // Instagram share
    const instagramBtn = document.getElementById('share-instagram-invite');
    if (instagramBtn) {
        const newBtn = instagramBtn.cloneNode(true);
        instagramBtn.parentNode.replaceChild(newBtn, instagramBtn);
        newBtn.addEventListener('click', shareViaInstagram);
    }
    
    // Messenger share
    const messengerBtn = document.getElementById('share-messenger-invite');
    if (messengerBtn) {
        const newBtn = messengerBtn.cloneNode(true);
        messengerBtn.parentNode.replaceChild(newBtn, messengerBtn);
        newBtn.addEventListener('click', shareViaMessenger);
    }
    
    // SMS share
    const smsBtn = document.getElementById('share-sms-invite');
    if (smsBtn) {
        const newBtn = smsBtn.cloneNode(true);
        smsBtn.parentNode.replaceChild(newBtn, smsBtn);
        newBtn.addEventListener('click', shareViaSMS);
    }
    
    // Email share
    const emailBtn = document.getElementById('share-email-invite');
    if (emailBtn) {
        const newBtn = emailBtn.cloneNode(true);
        emailBtn.parentNode.replaceChild(newBtn, emailBtn);
        newBtn.addEventListener('click', shareViaEmail);
    }
}
...
async function uploadFile(file, path) {
    ...
    const fileName = `${path}/${timestamp}_${random}.${extension}`;
...
async function testBase64Conversion() {
    ...
    try {
        ...
        showToast('Base64 conversion successful!', 'success');
    } catch (error) {
...
// Update loadProfilePage to include invite section
// Modify existing loadProfilePage function to call initializeInviteFriends
// Make it available globally
...
window.manageSlots = manageSlots;
window.closeFullDay = closeFullDay;
window.updateSlotStatus = updateSlotStatus;
window.bulkUpdateSlots = bulkUpdateSlots;
window.showPayoutRequestModal = showPayoutRequestModal;
window.requestPayout = requestPayout;
window.showPayoutConfirmationModal = showPayoutConfirmationModal;
window.showPayoutSuccessModal = showPayoutSuccessModal;
window.deleteGround = deleteGround;
window.deleteVenue = deleteVenue;
window.viewGround = viewGround;
