...
async function callCreateOrder(orderId, amount, paymentType, customerData) {
  const res = await fetch("https://us-central1-bookmygame-2149d.cloudfunctions.net/createOrder", {
    method : "POST",
...
function openCashfreePopup(paymentSessionId, orderId, paymentType, data, triggerBtn) {
  ...
  cashfree.checkout({
    ...
    onClose: () => {
      // User closed popup without completing — listener keeps running.
      // Webhook may still fire for bank-side completions.
